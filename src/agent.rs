                    ...
                    let handler = {
                        ...
                        move |data: Vec<u8>, worker: Value| {
                            ...
                            match msg {
                                FromWorker::WorkerLoaded => {
                                    // TODO Send `Connected` message
                                    let _ = DEDICATED_WORKERS_LOADED.with(|local| {
...
impl<AGN: Agent> PublicBridge<AGN> {
    ...
    fn worker_is_loaded(&self) -> bool {
        DEDICATED_WORKERS_LOADED.with(|local| local.borrow().contains(AGN::name_of_resource()))
    }
    fn msg_to_queue(&self, msg: Vec<u8>) {
        DEDICATED_WORKERS_EARLY_MSGS_QUEUE.with(|local| {
            match local.borrow_mut().entry(AGN::name_of_resource()) {
                ...
                std::collections::hash_map::Entry::Occupied(ref mut record) => {
