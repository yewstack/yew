    ...
    fn view(&self) -> Html {
        html! {
            <div>
                ...
                <button onclick=self.link.send_back(|_| Msg::SendToOpposite("One".into()))>{ "One" }</button>
                <button onclick=self.link.send_back(|_| Msg::SendToOpposite("Two".into()))>{ "Two" }</button>
                <button onclick=self.link.send_back(|_| Msg::SendToOpposite("Three".into()))>{ "Three" }</button>
                <button onclick=self.link.send_back(|_| Msg::SendToOpposite("Ping".into()))>{ "Ping" }</button>
            </div>
        }
    }
}
