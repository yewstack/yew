...
const VERSIONS = (async () => {
    const listedFiles = await fs.promises.readdir('versioned_docs', { withFileTypes: true })
...
async function checkSuperfluousTranslations() {
    ...
    for (const locale of locales) {
        ...
        for (const version of versions) {
            ...
            const originDir = isCurrentVersion ? 'docs' : path.join('versioned_docs', version);
            const localeDir = path.join('i18n', locale, 'docusaurus-plugin-content-docs', version);
            if (!await fs.promises.access(localeDir, fs.constants.F_OK).then(_ => true, _ => false)) {
                console.warn(`Missing translations for locale ${locale}, version ${version}.`)
                continue;
            }

            const result = await dircompare.compare(originDir, localeDir)
            if (!result.diffSet) { throw new Error('Expected diff set') }
            ...
            if (superfluous.length > 0) {
                ...
                severity(`Found superfluous translations for locale ${locale}, version ${version}:`, superfluous);
                if (isCurrentVersion)
                    success = false;
            }
        }
...
async function checkWriteTranslations() {
    const temp = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'yew-website-'))
