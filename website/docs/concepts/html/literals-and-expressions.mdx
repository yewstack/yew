...
For conditional rendering, use `if`/`else` and `match` directly in `html!` instead of wrapping them in expression blocks with nested `html!` calls:
...
It often makes sense to extract complex logic into functions or closures:
