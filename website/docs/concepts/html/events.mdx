...
-   [`Event::current_target`] points to the Yew subtree root instead of the element the listener is added on. Use
    [`NodeRef`](./node-refs.mdx) if you want access to the underlying `HtmlElement`.
...
[`NodeRef`](./node-refs.mdx) can be used instead of querying the event given to a `Callback`.
...
[`NodeRef`](./node-refs.mdx) so that in `use_effect_with_deps` we can add a listener using the
