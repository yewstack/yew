...
Firstly, we create a struct which defines what data is to be passed. For the above use-case, consider the following struct:
...
The children are re-rendered when the context changes. The `ContextProvider` can be used as:
