...
## `use_memo`
`use_memo` is used for obtaining an immutable reference to a value.
Its state persists across renders.
Its value will be recalculated if the dependency changes.
...
fn ref_hook() -> Html {
    let message = use_memo(|_| "Some Expensive State.".to_string(), ());
