...
This way Yew can be considered a very reactive framework as any change from the parent will allways be propogated downwards
and the view will never be out of sync from the data comming from props/state.
...
The atribute `#[function_component]` allows to optionaly specify Props in the function arguments
...
fn HelloWorld(props: &Props) -> Html {
    html! { <>{"Am I loading? - "}{*props.is_loading}</> }
}
