...
1. Using `String` type instead of `AttrValue`
    **Why is this bad?** `String` can be expensive to clone.
    Cloning is often needed when the prop value with hooks and such. `AttrValue` is either
    a reference-counted string (`Rc<str>`) or a `&'static str`, thus very cheap to clone.
2. Using interior mutability.
    **Why is this bad?** Interior mutability (such as with `RefCell`, `Mutex`, etc.) should
    *generally* be avoided. It can cause problems with re-renders (Yew doesn't know when state has changed)
