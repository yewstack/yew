...
pub fn show_storage_changed() -> Html {
...

    {
        ...
        use_effect(|| {
            let listener = EventListener::new(&window(), "storage", move || state_storage_changed.set(true));
...
View the docs on [docs.rs](https://docs.rc/yew) for documentation and `hooks` directory to see implementations of pre-defined hooks.
