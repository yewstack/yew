...
`<Portal />` component renders `children` into the `host` element provided in its properties.
...
Note that `<Portal />` is a low-level building block. Libraries should use it to implement
