...
Inspecting and Manipulating `Children` and `ChildrenRenderer` can often result in surprising and hard-to-explain behaviours in your application.
Trying to interpret and manipulate the `Children` can lead to edge cases and often do not yield expected result.
You should consider other approaches if you are trying to manipulate `Children`.

Yew now supports using `Html` as the type of the children prop.
You should use `Html` in your component if you do not need `Children` or `ChildrenRenderer`.
