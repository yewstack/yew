...
sg --pattern 'use_effect_with($CALLBACK,,$$$DEPENDENCIES)' --rewrite 'use_effect_with($CALLBACK,$$$DEPENDENCIES)' -l rs -i

sg --pattern 'use_callback($CALLBACK,$$$DEPENDENCIES)' --rewrite 'use_callback($$$DEPENDENCIES, $CALLBACK)' -l rs -i
sg --pattern 'use_callback($CALLBACK,,$$$DEPENDENCIES)' --rewrite 'use_callback($CALLBACK,$$$DEPENDENCIES)' -l rs -i

sg --pattern 'use_memo($CALLBACK,$$$DEPENDENCIES)' --rewrite 'use_memo($$$DEPENDENCIES, $CALLBACK)' -l rs -i
sg --pattern 'use_memo($CALLBACK,,$$$DEPENDENCIES)' --rewrite 'use_memo($CALLBACK,$$$DEPENDENCIES)' -l rs -i
```
