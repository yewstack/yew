...
## Replace `use_effect_with_deps` has been removed in favour of new `use_effect_with`

`use_effect_with` is same as `use_effect_with_deps` except the argument order is swapped.

### Automated refactor

With the help of https://crates.io/crates/ast-grep
Here is a command that can do the refactoring for you. It will leave duplicate commas, sometimes.

```bash
sg --pattern 'use_effect_with_deps($T,$$$DEPS)' --rewrite 'use_effect_with($$$DEPS $T)' -l rs -i
sg --pattern 'use_callback($T,$$$DEPS)' --rewrite 'use_callback($$$DEPS $T)' -l rs -i
```
