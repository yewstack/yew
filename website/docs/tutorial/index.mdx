...
Now, instead of hardcoding the list of videos in the HTML, let's define them as a `Vec` of Rust objects.
...
To display them, we need to convert these `Vec`s into `Html`. We can do that by creating an iterator,
