...
This is a simple hook which can be created by combining other hooks. For this example, we'll implement two pre-defined hooks.
...
Although this approach works in almost all cases, it can't be used to write primitive hooks like the pre-defined ones we've been using.

### Writing primitive hooks

The `use_hook` function is used to prepare this kind of hook. View the docs on [docs.rs](https://docs.rs/yew/0.18.0/yew-functional/use_hook.html) for the documentation, as well as the `hooks` directory to see implementations of pre-defined hooks.
