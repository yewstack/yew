...
在真实世界的应用程序中，数据通常会来自 API，而不是直接硬编码在程序里。现在让我们从外部来源获取影片列表。为此，我们需要添加以下 crates：

- [`gloo-net`](https://crates.io/crates/gloo-net)  
  用于发送 fetch 请求。
- [`serde`](https://serde.rs) 并启用 `derive` 功能  
  用于将 JSON 响应反序列化。
...
Yew 的 `serde` 功能会启用与 `serde` crate 的集成。对我们来说最重要的一点是，
它会为 `AttrValue` 添加 `serde::Deserialize` 的实现。

:::note
选择依赖项时，请务必确认它们与 `wasm32` 兼容！  
否则你的应用程序将无法运行。
:::
...
更新 `Video` 结构体，让它派生 `Deserialize` trait：
...
接下来我们需要更新 `App` 组件，让它能够获取数据。现代 Yew 的做法是使用 [`use_future`](https://docs.rs/yew/latest/yew/suspense/fn.use_future.html) 和 [`<Suspense>`](https://yew.rs/docs/concepts/suspense)。

另一种方式是使用 [`yew::platform::spawn_local`](https://docs.rs/yew/latest/yew/platform/fn.spawn_local.html)，
如果 hooks 无法使用，例如在 struct component 或普通函数中时，可以采用这种方式。

`use_future` 会在异步操作完成前挂起该组件，而 `<Suspense>` 则会在这段期间显示
后备 UI（例如加载中的提示）。
...
我们将数据获取逻辑拆分到一个子组件 `VideosFetcher` 中，该组件返回 `HtmlResult`，
而父组件 `App` 则使用 `<Suspense>` 将它包裹起来：
...
:::note
我们在 `use_future` 调用上使用 `?` 来传递 suspension。  
使用 suspense hooks 时，组件返回的是 `HtmlResult`，而不是 `Html`。父组件则通过 `<Suspense fallback=...>` 在数据获取期间显示加载提示。
:::

现在，回到浏览器查看，一切本来应该会如预期运行……如果不是因为 CORS 的话。  
要解决这个问题，我们需要一个代理服务器。幸运的是，trunk 已经提供了这个功能。
...
由于数据加载速度很快，你可能看不到加载中的文字。如果你想看到它，可以在 `use_future` hook 中使用 `TimeoutFuture` 添加一段人为延迟。首先，将 `gloo-timers` 添加到 `Cargo.toml`：
...
```rust
let videos = use_future(|| async {
+   gloo_timers::future::TimeoutFuture::new(3_000).await; // 延迟 3 秒
    Request::get("/tutorial/data.json")
        .send()
        .await?
        .json::<Vec<Video>>()
