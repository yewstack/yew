...
1. `html!` 巨集可以接受任意數量的根節點，包括零個。空的 `html! {}` 呼叫是有效的，不會渲染任何內容。
2. 字面量必須永遠用引號引起來並用大括號括起來：`html! { <p>{ "Hello, World" }</p> }`
3. `html!` 巨集會將所有標籤名稱轉換為小寫。若要使用大寫字元（某些SVG 元素所需的字元）請使用[動態標籤名稱](concepts/html/elements.mdx#dynamic-tag-names)：`html! { <@{"myTag"}>< /@> }`
...
可以使用 Rust 的 `if`、`if let` 和 `match` 運算式直接在 `html!` 中條件性地渲染標記。
