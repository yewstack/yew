...
在真實世界的應用程式中，資料通常會來自 API，而不是直接硬編碼在程式裡。現在讓我們從外部來源擷取影片清單。為此，我們需要加入以下 crates：

- [`gloo-net`](https://crates.io/crates/gloo-net)  
  用來發送 fetch 請求。
- [`serde`](https://serde.rs) 並啟用 `derive` 功能  
  用來將 JSON 回應反序列化。

讓我們更新 `Cargo.toml` 檔案中的相依套件：

```toml title="Cargo.toml" {2-5}
[dependencies]
-yew = { git = "https://github.com/yewstack/yew/", features = ["csr"] }
...
Yew 的 `serde` 功能會啟用與 `serde` crate 的整合。對我們來說最重要的一點是，
它會為 `AttrValue` 加上 `serde::Deserialize` 的實作。

:::note
選擇相依套件時，請務必確認它們與 `wasm32` 相容！  
否則你的應用程式將無法執行。
:::
...
更新 `Video` 結構，讓它衍生 `Deserialize` trait：
...
接著我們需要更新 `App` 元件，讓它能擷取資料。現代 Yew 的作法是使用 [`use_future`](https://docs.rs/yew/latest/yew/suspense/fn.use_future.html) 與 [`<Suspense>`](https://yew.rs/docs/concepts/suspense)。

另一種方式是使用 [`yew::platform::spawn_local`](https://docs.rs/yew/latest/yew/platform/fn.spawn_local.html)，
如果 hooks 無法使用，例如在 struct component 或一般函式中時，可以採用這種方法。

`use_future` 會在非同步操作完成前暫停該元件，而 `<Suspense>` 則會在這段期間顯示
備援 UI（例如載入中的提示）。
...
我們將資料擷取邏輯拆分到一個子元件 `VideosFetcher` 中，該元件回傳 `HtmlResult`，
而父元件 `App` 則使用 `<Suspense>` 將它包起來：
...
:::note
我們在 `use_future` 呼叫上使用 `?` 來傳遞 suspension。  
使用 suspense hooks 時，元件回傳的是 `HtmlResult`，而不是 `Html`。父元件則透過 `<Suspense fallback=...>` 在資料擷取期間顯示載入提示。
:::

現在，回到瀏覽器查看，一切本來應該會如預期運作……如果不是因為 CORS 的話。  
要解決這個問題，我們需要一個代理伺服器。幸運的是，trunk 已經提供了這個功能。
...
由於資料載入速度很快，你可能看不到載入中的文字。如果你想看見它，可以在 `use_future` hook 中使用 `TimeoutFuture` 加入一段人為延遲。首先，將 `gloo-timers` 加入 `Cargo.toml`：
...
```rust
let videos = use_future(|| async {
+   gloo_timers::future::TimeoutFuture::new(3_000).await; // 延遲 3 秒
    Request::get("/tutorial/data.json")
        .send()
        .await?
        .json::<Vec<Video>>()
