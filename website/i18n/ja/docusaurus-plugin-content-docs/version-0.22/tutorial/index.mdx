...
次に、データを取得できるように `App` コンポーネントを更新する必要があります。これを行う現代的な Yew の方法は、[`use_future`](https://docs.rs/yew/0.22.0/yew/suspense/fn.use_future.html) と [`<Suspense>`](https://yew.rs/docs/concepts/suspense) を使うことです。

また、フックが使えない場合、たとえば struct コンポーネントや通常の関数内では、代わりに [`yew::platform::spawn_local`](https://docs.rs/yew/0.22.0/yew/platform/fn.spawn_local.html) を使うこともできます。
