...
1. 式は中括弧（`{ }`）で囲む必要があります。
2. ルートノードは任意の数を返すことができ、自動的にまとめられます。
3. 要素は正しく閉じる必要があります。
...
Yew の `serde` 機能は `serde` クレートとの統合を有効にします。ここで重要なのは、`AttrValue` に対する `serde::Deserialize` の実装が追加される点です。
...
`Video` 構造体が `Deserialize` トレイトを derive するように更新します:
...
次に、データを取得できるように `App` コンポーネントを更新する必要があります。これを行う現代的な Yew の方法は、[`use_future`](https://docs.rs/yew/latest/yew/suspense/fn.use_future.html) と [`<Suspense>`](https://yew.rs/docs/concepts/suspense) を使うことです。

また、フックが使えない場合、たとえば struct コンポーネントや通常の関数内では、代わりに [`yew::platform::spawn_local`](https://docs.rs/yew/latest/yew/platform/fn.spawn_local.html) を使うこともできます。

`use_future` は非同期処理が完了するまでコンポーネントをサスペンドし、その間 `<Suspense>` はフォールバック UI（たとえばローディングインジケーター）を表示します。
...
データ取得ロジックは、`HtmlResult` を返す子コンポーネント (`VideosFetcher`) に分割し、親の `App` はそれを `<Suspense>` で包みます:
...
次に、この新しいコンポーネントを `App` コンポーネント内で使用します
...
:::note
`use_future` の呼び出しでは、サスペンド状態を伝播させるために `?` を使っています。サスペンス対応のフックを使う場合、コンポーネントは `Html` ではなく `HtmlResult` を返します。親コンポーネントはこれを `<Suspense fallback=...>` で包み、データ取得中にローディングインジケーターを表示します。
:::

では、ブラウザを見て、すべてが期待どおりに動作していることを確認しましょう……と言いたいところですが、CORS のせいでそうはいきません。

これを解決するには、プロキシサーバーが必要です。幸い、Trunk にはその機能が用意されています。
...
データの読み込みが非常に速いため、ローディングテキストが見えない場合があります。確認したい場合は、`TimeoutFuture` を使って `use_future` フック内に人工的な遅延を追加できます。まず、`Cargo.toml` に `gloo-timers` を追加します。
