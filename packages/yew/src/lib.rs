...
{
    start_app_with_props(element, COMP::Properties::default())
}

/// Starts an yew app mounted to the body of the document.
/// Alias to start_app(Body)
pub fn start_app_in_body<COMP>() -> AppHandle<COMP>
where
    COMP: Component,
    COMP::Properties: Default,
{
    start_app_in_body_with_props(COMP::Properties::default())
}
...
where
    ...
    COMP::Properties: Default,
{
    start_app_as_body_with_props(COMP::Properties::default())
}

/// The main entry point of a Yew application. This function does the
/// same as `start_app(...)` but allows to start an Yew application with properties.
pub fn start_app_with_props<COMP>(element: Element, props: COMP::Properties) -> AppHandle<COMP>
where
    COMP: Component,
{
...
/// need to manipulate the body element. For example, adding/removing app-wide
/// CSS classes of the body element.
pub fn start_app_as_body_with_props<COMP>(props: COMP::Properties) -> AppHandle<COMP>
where
    COMP: Component,
{
