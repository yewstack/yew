...
use crate::virtual_dom::vtag::{InputFields, VTagInner, SVG_NAMESPACE};
...
mod tests {
    ...
    fn supports_svg() {
        ...
        let g_node = html! { <@{"g"} class="segment"></@> };
        let path_node = html! { <@{"path"}></@> };
        let svg_node = html! { <@{"svg"}>{path_node}</@> };
    ...
    fn it_does_not_set_missing_class_name() {
        ...
        let elem = VNode::from(VTag::new("div"));
    ...
    fn controlled_input_synced() {
        ...
        // Initial state
        let elem = create_expected_vnode();
        let (_, mut elem) = Reconcilable::attach(elem, &scope, &parent, NodeRef::default());
        ...
        let next_elem = create_expected_vnode();
    ...
    fn uncontrolled_input_unsynced() {
        ...
        // Initial state
        let elem = VNode::from(VTag::new("input"));
        let (_, mut elem) = Reconcilable::attach(elem, &scope, &parent, NodeRef::default());
        let vtag = assert_btag_ref(&elem);
        ...
        let next_elem = VNode::from(VTag::new("input"));
        let elem_vtag = assert_vtag(next_elem);
