...
/// A muncher which extracts values from a state array stored via the history API.
pub struct PopStateMuncher {
    pub count: usize,
}

impl Muncher for PopStateMuncher {
    fn munch(&self, args: &mut Args) -> bool {
        let mut route = args.take_route();
        for _ in 0..self.count {
            if let Some(arr) = route.state.dyn_ref::<Array>() {
                args.push(arr.get(0));
                route.state = arr.slice(1, arr.length()).into();
            } else {
                args.push(mem::replace(&mut route.state, JsValue::UNDEFINED));
            }
        }
        args.store_route(route);
        true
    ...
    fn reconstitute(&self, args: &mut Args) {
        let mut route = args.take_route();
        for _ in 0..self.count {
            let new_item = args.pop().unwrap_or(JsValue::UNDEFINED);
            if let Some(arr) = route.state.dyn_ref::<Array>() {
                route.state = Array::of1(&new_item).concat(arr).into();
            } else if route.state.is_undefined() {
                route.state = Array::of1(&new_item).into();
            } else {
                route.state = Array::of2(&new_item, &route.state).into();
            }
        }
        args.store_route(route);
    }
