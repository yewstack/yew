...
use crate::{
    backend::DomBackend,
    html::{AnyScope, Component, ComponentUpdate, NodeRef, Scope, Scoped},
};
use std::any::TypeId;
...
mod vcomp {
    ...
    impl<REND: DomBackend> VDiff for VComp<REND> {
        fn detach(&mut self, _parent: &REND::Element) {
        ...
        fn apply(
            ...
            let placeholder: REND::Node = REND::get_document().create_text_node("").into();
...
mod mountable {
    use super::*;
    pub(crate) trait Mountable<REND: DomBackend> {
    ...
    impl<COMP: Component, REND: DomBackend> Mountable<REND> for PropsWrapper<COMP> {
        fn copy(&self) -> Box<dyn Mountable> {
