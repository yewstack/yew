...
use syn::{Item, FnArg, Ident, Block, Type, parse_macro_input};
use quote::quote;
        ...
        match parsed {
            Item::Fn(func) => {
                ...
                let props_type = match inputs.len() {
                    ...
                    _ => return Err(syn::Error::new_spanned(&func.sig, "Functional components take only one argument of props struct as value"))
                };
            ...
            _ => Err(syn::Error::new(proc_macro2::Span::call_site(), "`functional_component` can only be applied to functions"))
        }
    }
}
...
pub fn functional_component(attr: TokenStream, item: TokenStream) -> TokenStream {
    let function_name = Ident::new(&["Function", attr.to_string().as_str()].concat(), Span::call_site().into());
