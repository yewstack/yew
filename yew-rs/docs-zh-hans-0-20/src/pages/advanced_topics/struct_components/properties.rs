crate::doc_page!(
    ...
    Content::new(vec![
        ...
        p(vec![
            text("默认情况下，实现了 "),
            code("Properties"),
            text(" 的结构体中的字段是必需的。当缺少了该字段并且在 "),
            code("html!"),
            text(" 宏中创建了组件时，将返回编译错误。对于具有可选属性的字段，使用 "),
            code("#[prop_or_default]"),
            text(" 来使用该类型的默认值。要指定一个值，请使用 "),
            code("#[prop_or_else(value)]"),
            text("，其中 value 是该属性的默认值。例如，要将一个布尔值的默认值设置为 "),
            code("true"),
            text("，请使用属性 "),
        ...
        p(vec![
            text("如果可以的话，在你的 props 上派生 "),
            code("PartialEq"),
            text(" 通常是很有意义的。这使用了一个"),
            bold(vec![text("性能优化与最佳实践")]),
            text("部分解释了的技巧，可以更轻松地避免重新渲染。"),
        ]),
        h2(vec![text("Properties 的内存/速度开销")]),
        p(vec![text("记住组件的 "), code("view"), text(" 函数签名："),]),
        code_block(
            "rust",
...
pub struct LinkProps {
    ...
    active: bool,
        ...
        p(vec![
            ...
            code("view"),
            text(
                " 函数里提供的引用的所有权。这是在将引用传递给组件时隐式克隆引用完成的，\
                 以获得构成其 props 的有所有权的值。"
            ),
        ]),
        p(vec![text(
