crate::doc_page!(
    ...
    Content::new(vec![
        ...
        p![
            "当组件从它的父组件接收 props 时，",
            code("change"),
            " 方法将被调用。除了允许你更新组件的状态，还允许你返回一个布尔类型的值 ",
            code("ShouldRender"),
            " 来指示组件是否应该响应 props 的更改而重新渲染自身。",
        ],
        p![
            "重新渲染的开销很大，你应该尽量避免。一个通用的法则是，你只应该在 props \
             实际更改时重新渲染。以下代码块展示了此法则，如果 props 和先前的 props 不同，则返回 ",
            code("true"),
            "：",
        ],
        code_block(
...
lto = true"#
        ...
        p![
            "但是我们可以更进一步！对于任何实现了 ",
            code("PartialEq"),
            " 的项，可以使用一个 trait 和一个 blanket implementation 将这六行样板代码减少到一行。",
        ],
        code_block(
            "rust",
            r#"[unstable]
...
build-std-features = ["panic_immediate_abort"]"#
        ...
        p![
            "出于代码可读性的原因，将 ",
            code("html!"),
            " 各个部分的代码迁移到他们自己的函数中通常是有意义的，这样就可以避免在深层嵌套的 HTML \
             中出现代码块向右偏移。",
        ],
        h2!["纯组件 / 函数式组件"],
        p![
            ...
            "）来在 ",
            code("html!"),
            " 宏中使用，并且根据它们的实现，它们可以被记忆化 - 使用前面提到的 ",
            code("neq_assign"),
            " 逻辑来防止因为相同的 props 而重新渲染。",
        ],
        p!["Yew 没有原生支持纯组件或者函数式组件，但是可以通过外部库获取它们。"],
        p!["函数式组件尚不存在，但是从理论上来讲，可以通过使用 proc 宏和标注函数生成纯组件。"],
        h2!["Keyed DOM nodes when they arrive"],
        h2!["使用 Cargo Workspaces 进行编译速度优化"],
        p![
            "可以说，使用 Yew 的最大缺点是编译时间长。编译时间似乎与 ",
            code("html!"),
            " 宏块中的代码量相关。对于较小的项目，这通常不是什么大问题，但是对于跨多个页面的 web \
             应用程序，将代码拆分为多个 crates 以最大程度地减少编译器要做的工作通常是有意义的。",
        ],
        p![
            ...
            code("Html"),
            " 的大函数。在最好的情况下，你将从重新构建所有代码到只重新构建主 crate 和一个页面的 \
             crate。在最糟糕的情况下，当你在\"公共\" crate \
