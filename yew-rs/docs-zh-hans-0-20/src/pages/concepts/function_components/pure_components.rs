crate::doc_page!(
    ...
    Content::new(vec![
        p![
            ...
            " when the returned ",
            code("Html"),
            " is deterministically derived from its props, and its view function mutates no state \
             or has other side-effects.",
        ],
        p![
            "For example below is a pure component. For a given prop ",
            code("is_loading"),
    ...
    if props.is_loading {
        ...
        p![
            "You might wonder if a component can be impure if it does not use any globals, since \
             its just a function that is called every render. This is where the next topic comes \
             in - ",
            link!["/docs/0.20/concepts/function-components/hooks", "hooks"],
        ],
    ])
);
