pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        h3(vec![text("Reaches")]),
        ul(vec![
            li(vec![text("Public - There will exist at most one instance of a Public Agent at any given time. Bridges will spawn or connect to an already spawned agent in a web worker. When no bridges are connected to this agent, the agent will disappear.")]),
            li(vec![text("Private - Spawn a new agent in a web worker for every new bridge. This is good for moving shared but independent behavior that communicates with the browser out of components. When the connected bridge is dropped, the agent will disappear.")]),
            li(vec![text("Global (WIP)")]),
        ]),
        h2(vec![text("Communication between Agents and Components")]),
        h3(vec![text("Bridges")]),
        p(vec![text("A bridge allows bi-directional communication between an agent and a component. Bridges also allow agents to communicate with one another.")]),
        p(vec![
            text("A "),
            code("use_bridge"),
        ...
        p(vec![text("A dispatcher allows uni-directional communication between a component and an agent. A dispatcher allows a component to send messages to an agent.")]),
