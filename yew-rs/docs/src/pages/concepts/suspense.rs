pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        p![text("Suspense is a way to suspend component rendering whilst waiting a task to complete and a fallback (placeholder) UI is shown in the meanwhile.")],
        p![text("It can be used to fetch data from server, wait for tasks to be completed by an agent, or perform other background asynchronous task.")],
        p![text("Before suspense, data fetching usually happens after (Fetch-on-render) or before component rendering (Fetch-then-render).")],
        h3![text("Render-as-You-Fetch")],
        p![text("Suspense enables a new approach that allows components to initiate data request during the rendering process. When a component initiates a data request, the rendering process will become suspended and a fallback UI will be shown until the request is completed.")],
...
fn app() -> Html {
    ...
    html! {
        ...
        p![
            ...
            link!("/docs/advanced-topics/struct-components/hoc", text("Higher Order Component")),
            text(" to achieve suspense-based data fetching."),
        ],
        p![
            text("The "),
            link!("https://github.com/yewstack/yew/tree/master/examples/suspense/src/struct_consumer.rs", text("suspense example in the Yew repository")),
            text(" demonstrates how to use."),
        ],
        h2![text("Relevant examples")],
        ul![
            li![link!("https://github.com/yewstack/yew/tree/master/examples/suspense", text("Suspense"))],
        ],
    ])
}
