pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        p(vec![text("Suspense is a way to suspend component rendering whilst waiting a task to complete and a fallback (placeholder) UI is shown in the meanwhile.")]),
        p(vec![text("It can be used to fetch data from server, wait for tasks to be completed by an agent, or perform other background asynchronous task.")]),
        p(vec![text("Before suspense, data fetching usually happens after (Fetch-on-render) or before component rendering (Fetch-then-render).")]),
        h3(vec![text("Render-as-You-Fetch")]),
        p(vec![text("Suspense enables a new approach that allows components to initiate data request during the rendering process. When a component initiates a data request, the rendering process will become suspended and a fallback UI will be shown until the request is completed.")]),
...
fn use_user() -> SuspensionResult<User> {
    match load_user() {
        ...
        p(vec![
            ...
            text(" returns 2 values: the suspension context itself, and a suspension handle. The latter is the one responsible for signaling when to re-render the suspended components, it provides 2 interchangeable ways to do so:"),
        ]),
        ol(vec![
            li(vec![
                text("Calling its "),
...
fn app() -> Html {
    ...
    html! {
        ...
        p(vec![
            text("It's not possible to suspend a struct component directly. However, you can use a function component as a "),
            link("/docs/advanced-topics/struct-components/hoc", vec![text("Higher Order Component")]),
            text(" to achieve suspense-based data fetching."),
        ]),
        p(vec![
            text("The "),
            link("https://github.com/yewstack/yew/tree/master/examples/suspense/src/struct_consumer.rs", vec![text("suspense example in the Yew repository")]),
            text(" demonstrates how to use."),
        ]),
        h2(vec![text("Relevant examples")]),
        ul(vec![
            li(vec![link("https://github.com/yewstack/yew/tree/master/examples/suspense", vec![text("Suspense")])]),
        ]),
    ])
}
