...
fn Modal(props: &ModalProps) -> Html {
    ...
    create_portal(
        ...
        h2![text("SSR limitation")],
        p![
            text("Portals are "),
            bold![text("not rendered during server-side rendering")],
            text(". They require a live DOM host element ("),
            code("web_sys::Element"),
            text(") which is unavailable on the server. If you need to render content into "),
            code("<head>"),
            text(" during SSR, see the "),
            link![
                "/docs/advanced-topics/server-side-rendering#rendering-head-tags",
                text("head rendering section"),
            ],
            text(" in the SSR documentation."),
        ],
        h2![text("Further reading")],
        ul![li![link![
            "https://github.com/yewstack/yew/tree/master/examples/portals",
            text("Portals example"),
        ]]],
    ])
}
