...
async fn no_main() {
    ...
    println!("{}", rendered);
        ...
        admonition(AdmonitionType::Caution, Some("Web APIs are not available!"), vec![
            p(vec![
        ...
        h2(vec![text("Rendering "), code("<head>"), text(" Tags")]),
        p(vec![
        ...
        admonition(AdmonitionType::Info, None, vec![
            p(vec![
                ...
                link("https://docs.rs/bounce/latest/bounce/helmet/index.html", vec![text("the "), code("<Helmet/>"), text(" component from Bounce")]),
                text("."),
            ]),
        ]),
...
fn main() {
    ...
    renderer.hydrate();
        ...
        code_block("rust", r#"// Build using `wasm32-wasip1` target or `wasm32-wasip2` target.
