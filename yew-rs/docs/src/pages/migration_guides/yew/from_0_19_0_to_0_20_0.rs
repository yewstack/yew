    ...
    Content::new(vec![
        ...
        p(vec![
            ...
            link(
                ...
                vec![text("github issue")],
            ),
            text("."),
        ]),
        h2(vec![text("New Hooks and Function Components API")]),
        p(vec![text(
        ...
        h2(vec![text("Automatic Message Batching")]),
        p(vec![text(
            ...
            vec![
                p(vec![text(
                    "For struct components, this means that if you send 2 messages to 2 different \
                     components, they will not be guaranteed to be seen in the same order they \
                     are sent. If you send 2 messages to the same component, they will still be \
                     passed to the component in the order they are sent. The messages are not \
                     sent to the component immediately so you should not assume that when the \
                     component receives a message it still has the same state at the time the \
                     message is created.",
                )]),
                p(vec![
                    ...
                    code("use_state(_eq)"),
                    text(
                        " and the new value of that state depends on the previous value, you may \
                         want to switch to ",
                    ),
                    code("use_reducer(_eq)"),
                    text(
                        ". The new value of the state will not be visible / acknowledged until \
                         the next time the component is rendered. The reducer action works \
                         similar to messages for struct components and will be sent to the \
                         reducer function in the same order as they are dispatched. The reducer \
                         function can see all previous changes at the time they are run.",
                    ),
                ]),
            ],
        ),
        h2(vec![text("Yew Renderer")]),
        p(vec![
            code("start_app*"),
        ...
        h2(vec![code("ref"), text(" prop for Components")]),
        p(vec![
        ...
        h2(vec![code("changed"), text(" Method on Components")]),
        p(vec![
            text("The method "),
            code("fn changed()"),
            text(" has now a new argument to provide the old properties to the function."),
        ]),
        p(vec![text("The old method's signature was:")]),
        code_block_ignore(
            "rust",
            r#"fn changed(&mut self, ctx: &Context<Self>) -> bool"#,
        ),
        p(vec![text("The new method's signature is now:")]),
        code_block_ignore(
            "rust",
