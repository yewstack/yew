pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        h2(vec![text("サーバー設定")]),
        h3(vec![text("index.html をフォールバックとして提供する")]),
        p(vec![
            text("アプリケーションが "),
            link("/ja/docs/concepts/router", vec![text("Yew ルーター")]),
 ...
 TypeError: WebAssembly: Response has unsupported MIME type 'text/plain' expected 'application/wasm'"#,
        ...
        h2(vec![text("環境変数を使用して動作をカスタマイズする")]),
        p(vec![
            text(
