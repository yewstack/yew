pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        h2(vec![text("Props を使用する際の問題：\"Prop Drilling\"")]),
        p(vec![
        ...
        p(vec![text("以下の例を考えてみましょう。これは props を介してテーマを渡しています：")]),
...
fn App() -> Html {
    ...
    html! {
        ...
        h2(vec![text("コンテキストの使用")]),
        h3(vec![text("ステップ 1：コンテキストの提供")]),
        p(vec![
...
pub fn ThemedButton() -> Html {
    ...
    html! {
        ...
        h3(vec![text("ステップ 2：コンテキストの使用")]),
        h4(vec![text("関数コンポーネント")]),
        p(vec![
            code("use_context"),
            text(" フックは、関数コンポーネント内でコンテキストを使用するために使用されます。詳細については、"),
            link("https://yew-rs-api.web.app/next/yew/functional/fn.use_context.html", vec![text("use_context ドキュメント")]),
            text(" を参照してください。"),
        ]),
        h4(vec![text("構造体コンポーネント")]),
        p(vec![text("構造体コンポーネント内でコンテキストを使用するには、2つの方法があります：")]),
        ul(vec![
            li(vec![
            ...
            li(vec![
                ...
                link("https://github.com/yewstack/yew/tree/master/examples/contexts/src/struct_component_subscriber.rs", vec![text("構造体コンポーネントのコンシューマーとしての例")]),
                text(" を参照してください。"),
            ]),
        ]),
        h2(vec![text("使用シナリオ")]),
        p(vec![text("通常、ツリーの異なる部分のリモートコンポーネントでデータを使用する必要がある場合、コンテキストが役立ちます。以下はいくつかの例です：")]),
        ul(vec![
            li(vec![
                bold(vec![text("テーマ")]),
            ...
            li(vec![
                bold(vec![text("現在のユーザーアカウント")]),
                text("：多くの場合、コンポーネントは現在ログインしているユーザーを知る必要があります。コンテキストを使用して、現在のユーザーオブジェクトをコンポーネントに提供できます。"),
            ]),
        ]),
        h3(vec![text("コンテキストを使用する前の考慮事項")]),
        p(vec![text("コンテキストは非常に使いやすいですが、それが誤用/過度に使用される可能性もあります。複数のレベル深いコンポーネントに props を共有するためにコンテキストを使用できるからといって、必ずしもそうすべきではありません。")]),
        p(vec![
        ...
        h2(vec![text("子コンポーネントのコンテキスト値を変更する")]),
        p(vec![
        ...
        p(vec![
            link("https://github.com/yewstack/yew/tree/master/examples/contexts", vec![text("コンテキストの例")]),
