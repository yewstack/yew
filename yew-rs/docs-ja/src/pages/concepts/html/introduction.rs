pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        p![
            code("html!"),
            " マクロを使用すると、宣言的に HTML および SVG コードを記述できます。これは、JavaScript で HTML に似たコードを記述できる拡張機能である JSX に似ています。",
        ],
        p![bold!["重要な注意点"]],
        ol![
            li![
            ...
            li![
                "空の ",
                code("html! {}"),
                " 呼び出しは有効で、何もレンダリングしません",
            ],
            li![
                "リテラルは常に引用符で囲み、中括弧で囲む必要があります：",
        ...
        admonition![AdmonitionType::Note, None,
            p![
                code("html!"),
                " マクロはコンパイラのデフォルトの再帰制限に達する可能性があります。コンパイル エラーが発生した場合は、クレートのルートに ",
                code("#![recursion_limit=\"1024\"]"),
                " のような属性を追加して問題を解決してください。",
            ],
        ],
        h2!["タグ (Tags) 構造"],
        p!["タグ (Tags) は HTML タグに基づいています。コンポーネント、要素、およびリストはすべてこのタグ構文に基づいています。"],
        p![
            "タグは自己閉鎖 ",
            code("<... />"),
            " であるか、開始タグごとに対応する終了タグが必要です。",
        ],
        tabs!["Open - Close",
...
html! {
  <input id="my_input"> // <- 閉じタグがありません
        ...
        p!["複雑なネストされた HTML および SVG レイアウトを作成するのは依然として簡単です："],
        tabs!["HTML",
            tab!["HTML", "HTML",
                code_block("rust", r#"use yew::prelude::*;
...
html! {
    <svg width="149" height="147" viewBox="0 0 149 147" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        h2!["Lints"],
        p![
            "もし、Rust コンパイラの開発者バージョンを使用して Yew をコンパイルする場合、マクロは一般的な落とし穴について警告します。もちろん、安定版コンパイラを使用してリリースビルドを行う必要があるかもしれません（例えば、組織のポリシーでそうする必要がある場合など）。しかし、安定版ツールチェーンを使用している場合でも、",
            code("cargo +nightly check"),
            " を実行すると、HTML コードを改善する方法がいくつか示されるかもしれません。",
        ],
        p![
            "現在、これらのリントは主にアクセシビリティに関連しています。リントに関するアイデアがあれば、",
            link!["https://github.com/yewstack/yew/issues/1334", "この問題"],
            "に自由にコメントしてください。",
        ],
        h2!["属性とプロパティの指定"],
        p!["属性は通常の HTML と同じ方法で要素に設定されます："],
        code_block("rust", r#"use yew::prelude::*;
...
html! { <my-element ~property="abc" /> };"#),
        admonition![AdmonitionType::Tip, None,
            p!["値がリテラルの場合、値を囲む中括弧は省略できます。"],
        ],
        admonition![AdmonitionType::Note, Some("リテラルとは"),
            p![
                "リテラルは、Rust のすべての有効な",
                link!["https://doc.rust-lang.org/reference/expressions/literal-expr.html", "リテラル式"],
                "です。注意してください、",
                link!["https://users.rust-lang.org/t/why-are-negative-value-literals-expressions/43333", "負の数は", bold!["リテラルではありません"]],
                "、したがって中括弧で囲む必要があります ",
                code("{-6}"),
            ],
        ],
        ...
        h3!["特殊属性"],
        p![
            "いくつかの特殊な属性があり、これらは直接 DOM に影響を与えるのではなく、Yew 仮想 DOM の指示として機能します。現在、2 つの特殊な属性があります：",
            code("ref"),
            " と ",
            code("key"),
            "。",
        ...
        p![
            "一方、",
            code("key"),
            " は要素に一意の識別子を提供し、Yew が最適化のために使用できます。",
        ],
        ...
        h2!["条件付きレンダリング"],
        p![
            "Rust の条件構造を使用して、条件付きでマークアップをレンダリングできます。現在、",
            code("if"),
            " と ",
            code("if let"),
            " のみがサポートされています。",
