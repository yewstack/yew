...
fn MyComponent() -> Html {
    ...
    (*node).clone()
        ...
        h2_id!["dynamic-tag-names", "動的なタグ名"],
        p![
            "高階コンポーネントを構築する際、タグ名が静的ではない状況に遭遇することがあります。\
             例えば、",
            code("Title"),
            " コンポーネントがあり、レベル属性に応じて ",
            code("h1"),
            " から ",
            code("h6"),
            " までの任意の内容をレンダリングする場合です。大きなマッチ式を使用する代わりに、Yew \
             は ",
            code("@{name}"),
            " を使用してタグ名を動的に設定することを許可します。ここで、",
            code("name"),
            " は文字列を返す任意の式です。",
        ],
        code_block(
...
html! {
    <@{format!("h{}", level)} class="title">{ text }</@>
        ...
        h2!["論理値属性"],
        p!["いくつかのコンテンツ属性（例えば、checked、hidden、\
...
html! {
    <div>
        ...
        h2!["HTML 要素のオプション属性"],
        p!["ほとんどの HTML 属性はオプションの値（Some(x) または \
            None）を使用できます。これにより、\
