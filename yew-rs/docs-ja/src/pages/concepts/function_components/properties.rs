pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        h2!["リアクティブ性"],
        p!["再レンダリング時に、Yew は仮想DOMを調整する際にプロパティが変更されたかどうかを確認し、ネストされたコンポーネントを再レンダリングする必要があるかどうかを判断します。これにより、Yew は非常にリアクティブなフレームワークと見なされます。親コンポーネントからの変更は常に下位に伝播し、ビューはプロパティ/状態からのデータと常に同期します。"],
...
pub struct Props {
    pub is_loading: bool,
        ...
        p![
            "属性 ",
            code("#[component]"),
            " は、関数の引数で Props を選択的に受け取ることを可能にします。それらを提供するには、",
            code("html!"),
...
fn App() -> Html {
    html! { <HelloWorld /> }
        ...
        admonition!(AdmonitionType::Tip, None,
            p!["属性は Rustdoc によって生成されたドキュメントには表示されません。属性のドキュメント文字列には、その属性がオプションであるかどうか、および特定のデフォルト値があるかどうかを記載する必要があります。"],
        ),
        tabs!(
            "prop_or_default",
            tab!(
    ...
    html! { <Hello name="Sam" /> }
            ...
            tab!(
                ...
                p![
                    ...
                    " シグネチャを持つ必要があり、ここで ",
                    code("T"),
                    " はフィールドの型です。このプロパティに明示的な値が与えられていない場合、その関数が呼び出されます。この関数はプロパティが構築されるときに呼び出されます。",
                ],
                code_block(
                    "rust",
                    r#"use yew::prelude::*;
...
fn Case2() -> Html {
    html! { <Hello name="Sam" /> }
        ...
        p!["内部プロパティは参照カウントされたスマートポインタとして渡されます。これにより、コンポーネントツリー内のプロパティに対して共有ポインタが1つだけ渡されるため、プロパティ全体をクローンする高コストを節約できます。"],
        admonition!(AdmonitionType::Tip, None,
            p![
                code("AttrValue"),
...
fn main() {
    ...
    assert_eq!(props.last, 3);
        ...
        h2!["アンチパターン"],
        p!["ほとんどのRust型はプロパティとして渡すことができますが、避けるべきアンチパターンがいくつかあります。これらには以下が含まれますが、これに限定されません："],
        ol![
            li_blocks![
                p![
                    code("String"),
                ...
                p![
                    ...
                    " ",
                    code("String"),
                    " のクローンは高コストです。プロパティ値がフックやコールバックと一緒に使用される場合、通常クローンが必要です。",
                    code("AttrValue"),
                    " は参照カウントされた文字列 (",
                    code("Rc<str>"),
                    ") または ",
            ...
            li_blocks![
                ...
                p![
                    ...
                    " など）は ",
                    italic!["通常"],
                    " 避けるべきです。これにより再レンダリングの問題が発生する可能性があり（Yewは状態が変更されたことを認識しません）、手動で再レンダリングを強制する必要があるかもしれません。すべてのものと同様に、適切な使用場所があります。慎重に使用してください。",
                ],
            ],
            li_blocks![
                p![
            ...
            li_blocks![
                p!["新しい発見があるかもしれません。早く知っておきたかったエッジケースに遭遇しましたか？問題を作成するか、このドキュメントに修正のPRを提供してください。"],
            ],
        ],
        h2!["yew-autoprops"],
        p![
