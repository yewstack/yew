pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        p(vec![
            ...
            code("Html"),
            text(" オブジェクトを返す"),
            link("https://ja.wikipedia.org/wiki/%E7%B4%94%E9%96%A2%E6%95%B0", vec![text("純粋")]),
    ...
    if props.is_loading {
        ...
        admonition(AdmonitionType::Note, None, vec![
            p(vec![
                ...
                link("/ja/docs/concepts/html/literals-and-expressions#expressions", vec![text("式構文")]),
                text(" を使用して "),
                code("html!"),
                text(" 内でそれらをレンダリングします。"),
            ]),
        ]),
        h2(vec![text("非純粋コンポーネント")]),
        p(vec![
            text("コンポーネントがグローバル変数を使用しない場合、それが「純粋」でない可能性があるかどうか疑問に思うかもしれません。なぜなら、それは毎回レンダリングされる固定関数として呼び出されるだけだからです。 これが次のトピック - "),
            link("/ja/docs/concepts/function-components/hooks", vec![text("フック")]),
            text(" の出番です。"),
        ]),
    ])
}
