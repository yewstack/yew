pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        p![text("以前のスローガンをもう一度見てみましょう：")],
        blockquote![p![text(
            "Yew の核心思想は、再利用可能な UI 部分に必要なすべての内容を 1 つの場所 - Rust ファイルに集中させることです。"
        )]],
        p![text("この声明を完成させるために、アプリケーションのロジックとレンダリングの動作を定義する概念を導入します：\"コンポーネント\"。")],
        h2![text("コンポーネントとは？")],
        p![text("コンポーネントは Yew の構成要素です。")],
        p![text("それらは次のことを行うべきです：")],
        ul![
            li![
                link!("/ja/docs/concepts/function-components/properties", text("Props")),
                text(" の形式でパラメータを受け取る"),
            ],
            li![text("独自の状態を持つことができる")],
            li![text("ユーザーに見える HTML フラグメント（DOM）を計算する")],
        ],
        h2![text("Yew コンポーネントの 2 つのフレーバー")],
        p![text("現在、関数コンポーネントについて読んでいます - これは Yew を使い始めるときや、シンプルなレンダリングロジックを書くときにコンポーネントを書くための推奨方法です。")],
        p![
            text("もう一つの、より高度ですがアクセスしにくいコンポーネントの書き方があります - "),
            link!("/ja/docs/advanced-topics/struct-components", text("構造コンポーネント")),
            text("。それらは非常に詳細な制御を可能にしますが、ほとんどの場合、そこまで詳細な制御は必要ありません。"),
        ],
        h2![text("関数コンポーネントの作成")],
        p![
