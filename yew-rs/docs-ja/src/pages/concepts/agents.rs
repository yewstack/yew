pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        p![text("エージェント (Agents) は、タスクを Web Workers にオフロードする方法です。")],
        p![
        ...
        h3![text("範囲")],
        ul![
            li![text("公開 - 任意の時点で、公開エージェントのインスタンスは最大で1つだけです。ブリッジはWeb Worker内でエージェントを生成するか、既に生成されたエージェントに接続します。ブリッジがこのエージェントに接続されていない場合、エージェントは消滅します。")],
            li![text("私有 - 新しいブリッジごとにWeb Worker内で新しいエージェントを生成します。これは、ブラウザと通信する共有だが独立した動作をコンポーネントから移動するのに適しています。接続されたブリッジが破棄されると、エージェントは消滅します。")],
            li![text("グローバル (WIP)")],
        ],
        h2![text("エージェントとコンポーネント間の通信")],
        h3![text("通信ブリッジ (Bridges)")],
        p![text("通信ブリッジ（ブリッジ）は、コンポーネントとエージェント間の通信チャネルです。これにより、コンポーネントはエージェントにメッセージを送信し、エージェントからのメッセージを受信できます。")],
        p![
            code("use_bridge"),
            text(" フックは、関数コンポーネント内でブリッジを作成する機能も提供します。"),
        ],
        h3![text("ディスパッチャー (Dispatchers)")],
        p![text("ディスパッチャー（ディスパッチャー）は、コンポーネントとエージェント間の一方向通信を可能にし、コンポーネントがこの方法でエージェントにメッセージを送信します。")],
        h2![text("オーバーヘッド")],
        p![
