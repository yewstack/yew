pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        h2(vec![text("`web-sys` の継承")]),
        p(vec![
            link("/ja/docs/concepts/basic-web-technologies/wasm-bindgen#simulating-inheritance", vec![
                text("継承のシミュレーション"),
            ]),
        ...
        p(vec![
            ...
            link("https://doc.rust-lang.org/std/ops/trait.Deref.html#tymethod.deref", vec![
                code("Deref::deref"),
            ]),
            text(" を呼び出して、その値が "),
            link("/ja/docs/concepts/basic-web-technologies/wasm-bindgen#jsvalue", vec![
                code("JsValue"),
            ]),
...
fn inheritance_of_text_area(text_area: HtmlTextAreaElement) {
    ...
    let event_target: &EventTarget = text_area.as_ref();
        ...
        p(vec![
            link("https://wasm-bindgen.github.io/wasm-bindgen/web-sys/inheritance.html", vec![
                text("wasm-bindgen ガイドの web-sys 継承"),
            ]),
        ]),
        h2(vec![text("`NodeRef` の `Node`")]),
        p(vec![
            text("Yew は "),
            ...
            code("Node"),
            text(" は "),
            link("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.Node.html", vec![
                code("web_sys::Node"),
            ]),
            text(" を指します。"),
            code("NodeRef::get"),
            ...
            link("/ja/docs/concepts/basic-web-technologies/wasm-bindgen#jscast", vec![
                code("JsCast"),
            ]),
            text(" を使用して "),
            code("Node"),
...
fn with_jscast(node_ref: NodeRef) {
    ...
    console.log('Left? : ' + x + ' ; Top? : ' + y + '.')
        ...
        h3(vec![text("`web-sys` を使用して書き直した例")]),
        p(vec![
            code("web-sys"),
...
html! {
    <div id=\"mousemoveme\" {onmousemove}></div>
        ...
        p(vec![
            italic(vec![
                link("/community/external-libs", vec![text("追加の依存ライブラリ一覧")]),
            ]),
        ]),
    ])
}
