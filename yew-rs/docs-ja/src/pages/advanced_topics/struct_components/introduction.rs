    ...
    Content::new(vec![
        ...
        admonition(AdmonitionType::Info, None, vec![
            p(vec![link(
                ...
                vec![text("html! マクロ、その使用方法、および構文についてさらに詳しく知る")],
            )]),
        ]),
        h2(vec![text("コンポーネントにデータを渡す")]),
        p(vec![
            text("Yew コンポーネントは "),
        ...
        admonition(AdmonitionType::Info, None, vec![
            p(vec![link(
                ...
                vec![text("props についてさらに詳しく知る")],
            )]),
        ]),
        admonition(AdmonitionType::Info, None, vec![
            p(vec![
                text("親/子通信以外の通信には、"),
                link("/ja/docs/concepts/contexts", vec![text("コンテキスト")]),
                text(" を使用してください"),
            ]),
        ]),
    ])
