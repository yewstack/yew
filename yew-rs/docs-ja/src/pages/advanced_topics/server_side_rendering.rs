pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        h1(vec![text("サーバーサイドレンダリング (Server-Side Rendering)")]),
        p(vec![text("デフォルトでは、Yewコンポーネントはクライアントサイドでレンダリングされます。ユーザーがウェブサイトにアクセスすると、サーバーは実際のコンテンツを含まない骨組みのHTMLファイルとWebAssemblyパッケージをブラウザに送信します。すべてのコンテンツはクライアントサイドでWebAssemblyパッケージによってレンダリングされます。これをクライアントサイドレンダリングと呼びます。")]),
        p(vec![text("この方法はほとんどのウェブサイトにとって有効ですが、いくつかの注意点があります：")]),
        ol(vec![
            li(vec![text("ユーザーはWebAssemblyパッケージがダウンロードされ、初期レンダリングが完了するまで何も表示されません。これにより、ネットワークが遅い場合にユーザーエクスペリエンスが悪化する可能性があります。")]),
            li(vec![text("一部の検索エンジンは動的にレンダリングされたウェブページのコンテンツをサポートしておらず、サポートしている検索エンジンでも通常は動的なウェブサイトのランキングが低くなります。")]),
        ]),
        p(vec![text("これらの問題を解決するために、ウェブサイトをサーバーサイドでレンダリングすることができます。")]),
        h2(vec![text("動作原理")]),
        p(vec![
...
async fn no_main() {
    ...
    println!("{}", rendered);
        ...
        h2(vec![text("コンポーネントのライフサイクル")]),
        p(vec![text("クライアントサイドレンダリングとは異なり、サーバーサイドレンダリング時のコンポーネントのライフサイクルは異なります。")]),
        p(vec![
            text("コンポーネントが最初に "),
            code("Html"),
        ...
        admonition(AdmonitionType::Caution, Some("ブラウザインターフェースは利用できません！"), vec![
            p(vec![
                code("web_sys"),
        ...
        admonition(AdmonitionType::Danger, Some("構造化コンポーネント"), vec![
            p(vec![
            ...
            p(vec![text("サーバーサイドレンダリングをサポートするアプリケーションを設計する際は、特別な理由がない限り、関数コンポーネントを使用することをお勧めします。")]),
        ]),
        h2(vec![text("サーバーサイドレンダリング中のデータ取得")]),
        p(vec![text("データ取得はサーバーサイドレンダリングとハイドレーション（hydration）中の難点の一つです。")]),
        p(vec![text("従来の方法では、コンポーネントがレンダリングされるとすぐに利用可能になります（仮想DOMを出力してレンダリングします）。コンポーネントがデータを取得する必要がない場合、この方法は有効です。しかし、コンポーネントがレンダリング時にデータを取得しようとするとどうなるでしょうか？")]),
        p(vec![text("以前は、Yewにはコンポーネントがまだデータを取得しているかどうかを検出するメカニズムがありませんでした。データ取得クライアントは、初期レンダリング中に何が要求されたかを検出し、要求が完了した後に再レンダリングをトリガーするソリューションを実装する責任がありました。サーバーはこのプロセスを繰り返し、応答を返す前にレンダリング中に追加の保留中の要求がないことを確認します。")]),
        p(vec![text("これは、コンポーネントを繰り返しレンダリングするため、CPUリソースを浪費するだけでなく、データクライアントは、サーバー側で取得したデータをハイドレーション中に利用可能にする方法を提供する必要があり、初期レンダリングで返される仮想DOMがサーバーサイドレンダリングのDOMツリーと一致することを保証する必要があります。これは実現が難しい場合があります。")]),
        p(vec![
            text("Yewは、"),
            code("<Suspense />"),
        ...
        p(vec![text("アプリケーションがサーバーサイドレンダリングされると、Yewはコンポーネントが保留状態でなくなるまで待機し、それを文字列バッファにシリアル化します。")]),
        p(vec![
            text("ハイドレーション中、"),
            code("<Suspense />"),
            text(" コンポーネント内の要素は、すべての子コンポーネントが保留状態でなくなるまでハイドレーションされません。"),
        ]),
        p(vec![text("この方法により、開発者はサーバーサイドレンダリングに対応したクライアント非依存のアプリケーションを簡単に構築し、データ取得を行うことができます。")]),
        h2(vec![text("`<head>` タグのレンダリング")]),
        p(vec![
        ...
        p(vec![
            ...
            bold(vec![text("Yew の外部でサーバー側に")]),
            text("生成し、クライアントに送信する前に HTML テンプレートに埋め込む必要があります。"),
        ]),
        p(vec![
            link("https://github.com/yewstack/yew/blob/master/examples/ssr_router/src/bin/ssr_router_server.rs", vec![text("ssr_router サンプル")]),
        ...
        admonition(AdmonitionType::Info, None, vec![
            p(vec![
                ...
                link("https://docs.rs/bounce/latest/bounce/helmet/index.html", vec![text("Bounce の <Helmet/> コンポーネント")]),
                text(" が利用できます。"),
            ]),
        ]),
        h2(vec![text("サーバーサイドレンダリングハイドレーション（SSR Hydration）")]),
        p(vec![
        ...
        admonition(AdmonitionType::Warning, None, vec![
            p(vec![
                ...
                link("https://github.com/yewstack/yew/issues/2684", vec![text("<tbody> のない <table> がある場合、ブラウザはDOMに <tbody> を追加する可能性があります")]),
                text("。"),
            ]),
        ]),
        h2(vec![text("ハイドレーション中のコンポーネントライフサイクル")]),
        p(vec![
    ...
    println!("{}", render().await);
        ...
        admonition(AdmonitionType::Caution, None, vec![
            p(vec![
                ...
                link("https://github.com/yewstack/yew/issues/new?assignees=&labels=bug&template=bug_report.md&title=", vec![text("GitHubで報告してください")]),
                text("。"),
            ]),
        ]),
    ])
