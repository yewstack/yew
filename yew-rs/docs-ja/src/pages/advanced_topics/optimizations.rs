    ...
    Content::new(vec![
        ...
        p![
            bold![
                ...
                link!["https://doc.rust-lang.org/book/ch15-00-smart-pointers.html", text("スマートポインタに関する章")],
                text("が役立ちます。"),
            ],
        ],
        p![
        ...
        p![
            ...
            link!["https://doc.rust-lang.org/stable/std/rc/struct.Rc.html#method.ptr_eq", text("Rc::ptr_eqに関する詳細")],
            text("があります。"),
        ],
        p![
            text("この最適化は、"),
        ...
        p![text("ただし、子コンポーネントでデータを自分でクローンする必要がない限り、この最適化は無駄であり、不要な参照カウントのコストを追加するだけです。Yewのpropsはすでに参照カウントされており、内部でデータのクローンは行われません。")],
        h2![text("レンダリング関数")],
        p![
            text("コードの可読性のために、"),
            code("html!"),
        ...
        p![text("メインクレートが重すぎる場合や、深くネストされたページ（例：別のページ上にレンダリングされるページ）を迅速に反復したい場合は、メインページの簡略化された実装を作成し、作業中のコンポーネントを追加でレンダリングするためにサンプルクレートを使用できます。")],
        h2![text("バイナリサイズの縮小")],
        ul![
            li![text("Rustコードの最適化")],
            li![
        ...
        p![
            bold![
                ...
                link!["https://rustwasm.github.io/book/reference/code-size.html#optimizing-builds-for-code-size", text("Rust Wasmマニュアル")],
                text("を参照してください。"),
            ],
        ],
        h3![text("Cargo.toml")],
...
lto = true"#),
        ...
        p![
            ...
            link!["https://doc.rust-lang.org/cargo/reference/unstable.html", text("不安定な機能")],
            text("のドキュメント、特に"),
...
build-std-features = ["panic_immediate_abort"]"#),
        admonition![AdmonitionType::Caution, None,
            p![
                ...
                link!["https://github.com/yewstack/yew/issues/2696", text("この例")],
                text("のようなバグが含まれている可能性があるため、定期的に監視し調整する必要があります。これらの実験的なオプションを使用する際は注意が必要です。"),
            ],
        ],
        h3![text("wasm-opt")],
        p![text("さらに、"), code("wasm"), text(" コードのサイズを最適化することができます。")],
        p![
            text("Rust Wasm マニュアルには、Wasm バイナリファイルのサイズを縮小する方法に関するセクションがあります："),
            link!["https://rustwasm.github.io/book/game-of-life/code-size.html", text(".wasm サイズの縮小")],
        ],
        ul![
            li![
                code("wasm-pack"),
        ...
        h4![text("yew/examples/ の 'minimal' サンプルのビルドサイズ")],
        p![
            text("注意："),
            code("wasm-pack"),
        ...
        h2![text("さらに読む")],
        ul![
            li![link!["https://doc.rust-lang.org/book/ch15-00-smart-pointers.html", text("Rust マニュアルのスマート ポインターに関する章")]],
            li![link!["https://rustwasm.github.io/book/reference/code-size.html#optimizing-builds-for-code-size", text("Rust Wasm マニュアルのコードサイズの縮小に関する章")]],
            li![link!["https://doc.rust-lang.org/cargo/reference/profiles.html", text("Rust プロファイルに関するドキュメント")]],
            li![link!["https://github.com/WebAssembly/binaryen", text("binaryen プロジェクト")]],
        ],
    ])
}
