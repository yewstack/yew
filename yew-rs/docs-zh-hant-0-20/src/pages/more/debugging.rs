crate::doc_page!(
    ...
    Content::new(vec![
        ...
        p![
            ...
            " crate ，他會用 Rust symbols 來做 stacktraces。注意，他跟 ",
            code("cargo-web"),
            " 不相容。",
        ],
        h2!["Console Logging"],
        p![
            "通常，Wasm 的網頁應用程式可以跟瀏覽器的 API 互操作，所以 ",
            code("console.log"),
            " 這個 api 也不例外，你可以使用以下幾種方法：",
        ],
        h3!["wasm-logger"],
        p!["這個 crate 整合了令人熟悉的 Rust ", code("log"), " crate：",],
        code_block(
            "rust",
            r#"use log::info;
...
fn main() {
    ...
    info!("Hello {}", object.as_string().unwrap());
        ...
        h3!["ConsoleService"],
        p![
            "Yew 包含了這個 service，而且如果 ",
            code("\"services\""),
            " 這個 feature 有被打開的話，你可以直接使用他：",
        ],
        code_block(
            "rust",
...
fn main() {
    ...
    log!("Hello", object)
        ...
        h2!["Source Maps"],
        p!["目前 Rust/Wasm 網頁應用程式，不對 source maps \
        ...
        blockquote![p!["但還是有大量的工作要做。舉例還說，在工具方面，\
                        Emscripten (Binaryen) 與 wasm-pack \
                        (wasm-bindgen)，還不支援更新轉換他們的行為的 DWARF \
                        資訊。"]],
        p![
            "[2020] ",
            link!(
        ...
        blockquote![p![
            "不幸地，WebAssembly 的除錯還不夠完善。在大部分的 Unix 系統中，",
        ...
        blockquote![p!["除錯是一件棘手的事情，\
                        因為大部分的事情都不是掌握在這個工作群組中，而是依賴 \
