crate::doc_page!(
    ...
    Content::new(vec![
        ...
        p![
            "Router 元件會與 ",
            code("RouteAgent"),
            " 溝通，並且自動解析從 agent 到 switch 的 Routes，Routes 會在 render 的 \
             屬性中被揭露，這個屬性會決定 switch 的結果如何被轉換成 ",
            code("Html"),
            "。",
        ],
        h2!["如何使用路由器"],
...
enum AppRoute {
  ...
  ViewPosts,
        ...
        p![
            ...
            code("{<number>}"),
            " 表示取得特定數量的的分隔符號之前的變數。（例如： ",
            code("{2}"),
            " 會取得兩個分隔符號之前的變數。）",
        ],
        p![
            "對於有命名欄位的 struct 與 enum，你必須給出變數的名字，像是： ",
            code("{user_name}"),
            " 或是 ",
            code("{*:age}"),
            "。",
