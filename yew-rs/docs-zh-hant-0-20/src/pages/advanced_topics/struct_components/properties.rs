crate::doc_page!(
    ...
    Content::new(vec![
        ...
        p![
            "預設所有在 ",
            code("Properties"),
            " struct 裡的欄位都是必填的。當必填的欄位沒有值，而元件在 ",
            code("html!"),
            " 巨集中又被建立，編譯器就會報錯。如果希望欄位是可選的，可以使用 ",
            code("#[prop_or_default]"),
            " 來讓該欄位有預設值。如果希望欄位預設特定值，可以使用 ",
            code("#[prop_or_else(value)]"),
            " ，裡面的 value 就會是這個欄位的預設值。舉例來說，希望預設值是 ",
            code("true"),
            "可以在欄位宣告上面這樣寫 ",
        ...
        h3!["PartialEq"],
        p![
            "如果可以，最好在你的屬性上面 derive ",
            code("PartialEq"),
            " 。他可以避免畫面多餘的渲染，更細節的內容請參考，",
            bold!["優化與最佳實例"],
            "的區塊。",
        ],
        h2!["屬性的記憶體與速度的開銷"],
        p![
            "在 ",
            code("Component::view"),
            ",裡，你可以拿到元件狀態的參考，且用他來建立 ",
            code("Html"),
            " 。但是屬性是有所有權的。這代表著為了建立屬性，並且將他們傳遞給子元件，\
             我們必須取得被 ",
            code("view"),
            " 方法拿走的所有權。 當將參考傳給元件時，可以透過隱式的複製來做到得到所有權。",
        ],
        p![
