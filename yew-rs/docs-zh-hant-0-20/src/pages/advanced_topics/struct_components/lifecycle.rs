crate::doc_page!(
    ...
    Content::new(vec![
        ...
        h3!["Create"],
        p![
            "當一個元件被建立，他會接收從父元件，也就是 ",
            code("ComponentLink"),
            " ，傳下來的屬性。 這些屬性用來初始化元件的狀態，此外，\
             「link」可以用來註冊回調函式或傳訊息給元件。",
        ],
        p!["通常，你的元件 struct 會儲存 props 與 link，就像下面的例子："],
        code_block(
            "rust",
...
impl Component for MyComponent {
    ...
    fn view(&self, _ctx: &Context<Self>) -> Html {
        ...
        p![
            ...
            " 方法中宣告佈局。Yew 提供 ",
            code("html!"),
            " 巨集來宣告 HTML 合 SVG 的結點，包含他們的監聽事件與子結點。這個巨集扮演像是 React \
             的 JSX 的角色，但是是使用 Rust 的表達式，而不是 JavaScript 的。",
        ],
        code_block(
            "rust",
            r#"use yew::{Component, Context, html, Html, Properties};
...
impl Component for MyComponent {
    ...
    // highlight-end
        ...
        admonition![
            AdmonitionType::Note,
            None,
            p!["注意，這個生命周期方法，不是一定要被實作，預設的行為是不做任何事情。"],
        ],
        h3!["Update"],
        p![
            "元件是可動態更新且可以註冊接收非同步的訊息。 ",
            code("update()"),
...
impl Component for Comp {
    ...
    // ...
        ...
        p![
            code("Message"),
            " 負責各式各樣的訊息，他可能被元件處理去觸發各種影響。舉例來說，你可能有一個 ",
            code("Click"),
            " 的訊息，他會觸發 API 請求，或是切換 UI \
             元件的樣貌。下面是一個常見的實作，在你的元件模組中，創建一個叫作 ",
            code("Msg"),
            " 的 enum，然後把他當作元件裡的 Message 型別。通常 message 會縮寫成 msg。",
        ],
        code_block(
            "rust",
            r#"enum Msg {
    ...
    FormInput(String)
        ...
        p![
            code("Properties"),
            " 代表要從父員件傳遞到子元件的資訊。這個型別必須實作 ",
            code("Properties"),
            " trait （通常會 deriving \
             他）並且可以決定某個屬性是必要的屬性，或是可選的屬性。這個型別會在創建元件的時候，\
             或是更新元件的時候被使用到。常見的實作會在你的元件模組中，建立一個叫作 ",
            code("Props"),
            " 的 struct，然後把他當作元件的",
            code("Properties"),
