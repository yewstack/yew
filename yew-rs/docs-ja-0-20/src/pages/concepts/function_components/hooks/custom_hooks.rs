crate::doc_page!(
    ...
    Content::new(vec![
        ...
        p(vec![text(
            "Component's stateful logic can be extracted into reusable function by creating \
             custom Hooks."
        )]),
        p(vec![
            text(
    ...
    html! { <div>{"Storage Event Fired: "}{*state_storage_changed}</div> }
        ...
        p(vec![text(
            "There's one problem with this code: the logic can't be reused by another component. \
             If we build another component which keeps track of the an event, instead of copying \
    ...
    F: Fn(&Event) + 'static,
{
    todo!()
        ...
        p(vec![
            text(
                "This is a simple hook which can be created by composing built-in hooks. For this \
