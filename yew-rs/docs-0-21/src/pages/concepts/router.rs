        ...
        p![
            ...
            text(
                ", and push the targeted page to the history and render the desired page, which \
                 is what should be expected from a Single Page App. The default "
            ),
            code("onclick"),
        ...
        p![
            ...
            text(
                " hook re-renders the component when the underlying navigator provider changes. \
                 Here is how to implement a button that navigates to the "
            ),
            code("Home"),
            ...
            p![
                ...
                text(
                    ". Use a normal callback if the target route can be the same as the route the \
                     component is in, or just to play safe. For example, consider a logo button \
                     on every page that goes back to the home page when clicked. Clicking that \
                     button twice on the home page causes the code to panic because the second \
                     click pushes an identical Home route and the "
                ),
                code("use_navigator"),
                text(" hook will not trigger a re-render."),
            ]
        ),
        p![
            text(
...
fn some_page() -> Html {
    ...
    MyComponent {
        ...
        p![
            ...
            code("serde"),
            text(" to deserialize the parameters from the query string in the URL."),
        ],
