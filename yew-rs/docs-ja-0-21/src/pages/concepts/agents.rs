crate::doc_page!("Agents", "/ja/docs/concepts/agents",
    Content::new(vec![
        p(vec![
            ...
            link("https://angular.io/guide/architecture-services", vec![text("サービス")]),
            text("に似ており(ただし依存性インジェクションはありません)、"),
            link("https://en.wikipedia.org/wiki/Actor_model", vec![text("アクターモデル")]),
            text("を提供します。\
        ...
        ul(vec![
            ...
            li(vec![text("Public - Context と同じですが、独自の web worker で動作します。")]),
            li(vec![text("Private - Job と同じですが、独自の web worker で動作します。")]),
            li(vec![text("Global (WIP)")]),
        ]),
        h2(vec![text("エージェントとコンポーネントのやり取り")]),
        h3(vec![text("Bridges")]),
        p(vec![
