crate::blog_page!(
    ...
    Content::new(vec![
        ...
        p(vec![
            text(
                "Since the old SSR implementation will create new tasks directly in the \
                 asynchronous context directly (based on "
            ),
            code("prokio"),
            text(
                "). It only allowed in a multi-threaded environment that it is not compatible \
                 with WASI. So the new version added a dedicated one for a single-threaded \
                 environment that rendering function to support single-threaded scenes."
            ),
        ]),
        p(vec![
            text("Learn more at "),
            link(
                ...
                vec![text("Server-side rendering example on WASI environment"),]
            ),
        ]),
