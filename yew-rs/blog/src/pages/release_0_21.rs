crate::blog_page!(
    ...
    // Do something with dependencies
        ...
        p(vec![
            text("This is a big breaking change so we've provided "),
            link(
                "https://yew.rs/docs/migration-guides/yew/from-0_20_0-to-0_21_0#automated-refactor",
                vec![text("a way to automate the refactor"),]
            ),
        ]),
        h3(vec![text("2. Versatile Child Types")]),
        p(vec![
            text(
                "Yew now allows you to use any type as children within your components. This \
                 means you're no longer limited to just the "
            ),
            code("Children"),
            text(" type. Instead, you can use any type, even "),
            code("Html"),
            text(" or closures, unlocking patterns such as:"),
        ]),
        code_block(
            "rust",
            r##"html! {
    <Comp>
        ...
        h3(vec![text("3. Agents: A Complete Rewrite")]),
        p(vec![
            text("Yew 0.21 brings a complete rewrite of "),
            code("yew-agent"),
            text(
                ". This streamlines and simplifies the way workers operate. Here's what you need \
                 to know:"
            ),
        ]),
        ul(vec![
            li(vec![
                bold(vec![text("Introducing Providers:")]),
                text(" Say goodbye to the old "),
                code("Worker::bridge()"),
                text(" method. Now, you can use the "),
                code("WorkerProvider"),
                text(" / "),
                code("ReactorProvider"),
                text(" / "),
                code("OneshotProvider"),
                text(" as per your need, by creating them using the hooks."),
            ]),
            li(vec![
                bold(vec![text("WorkerLink to WorkerScope:")]),
                text(
                    " We've removed WorkerLink in favor of WorkerScope. This change simplifies \
                     the worker architecture, making it more straightforward to manage and \
                     maintain."
                ),
            ]),
        ]),
        ...
        ul(vec![
            li(vec![
                bold(vec![text("Worker Agent:")]),
                text(
                    " The original agent that uses an actor model, designed to handle complex \
                     states."
                ),
            ]),
            li(vec![
                bold(vec![text("Oneshot Agent:")]),
                text(
                    " Designed for scenarios where you expect a single input and a single output \
                     for each agent."
                ),
            ]),
            li(vec![
                bold(vec![text("Reactor Agent:")]),
                text(
                    " Ideal for situations where multiple inputs and multiple outputs are needed \
                     for each agent."
                ),
            ]),
        ]),
        p(vec![
            ...
            link(
                ...
                vec![text("documentation of yew-agent")]
            ),
        ]),
        h3(vec![text(
        ...
        h2(vec![text("Call for Contributors")]),
        p(vec![text(
        ...
        p(vec![text("Here are some areas where you can contribute:")]),
        ul(vec![
            li(vec![
                bold(vec![text("Code Contributions:")]),
                text(
            ...
            li(vec![
                bold(vec![text("Documentation:")]),
                text(
                    " Clear and comprehensive documentation is vital for any project's success. \
                     You can contribute by improving documentation, writing tutorials, or \
                     creating examples that help others understand and use Yew effectively."
                ),
            ]),
            li(vec![
                bold(vec![text("Testing and Bug Reporting:")]),
                text(
                    " Testing Yew and reporting bugs you encounter is a valuable contribution. \
                     Your feedback helps us identify and fix issues, ensuring a more stable \
                     framework for everyone."
                ),
            ]),
            li(vec![
                bold(vec![text("Community Support:")]),
                text(
                    " Join discussions, chat rooms (we have our own Discord and Matrix!), or \
                     social media to assist other developers using Yew. Sharing your knowledge \
                     and helping others solve problems is a fantastic way to contribute."
                ),
            ]),
        ]),
        p(vec![text(
