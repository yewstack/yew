pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        h1(vec![text("服务端渲染 (Server-Side Rendering)")]),
        p(vec![
        ...
        p(vec![text("为了解决这些问题，我们可以在服务端渲染我们的网站。")]),
        h2(vec![text("工作原理")]),
        p(vec![
            text("Yew 提供了一个 "),
            code("ServerRenderer"),
...
async fn no_main() {
    ...
    println!("{}", rendered);
        ...
        h2(vec![text("渲染 "), code("<head>"), text(" 标签")]),
        p(vec![
        ...
        p(vec![
            ...
            code("<head>"),
            text("。因此，head 标签必须"),
            bold(vec![text("在服务端、Yew 之外")]),
            text("生成，并在发送给客户端之前拼接到 HTML 模板中。"),
        ]),
        p(vec![
            link("https://github.com/yewstack/yew/blob/master/examples/ssr_router/src/bin/ssr_router_server.rs", vec![code("ssr_router"), text(" 示例")]),
        ...
        admonition(
            ...
            vec![
                p(vec![
                    ...
                    link("https://docs.rs/bounce/latest/bounce/helmet/index.html", vec![text("Bounce 的 "), code("<Helmet/>"), text(" 组件")]),
                    text("。"),
                ]),
            ],
        ),
        ...
        admonition(
            ...
            vec![
                p(vec![
                    ...
                    link("https://github.com/yewstack/yew/issues/2684", vec![text("如果您有一个没有 "), code("<tbody>"), text(" 的 "), code("<table>"), text("，浏览器可能会向 DOM 添加一个 "), code("<tbody>")]),
                ]),
            ],
        ),
        h2(vec![text("水合期间的组件生命周期")]),
        p(vec![
        ...
        admonition(
            ...
            vec![
                p(vec![
                    ...
                    link("https://github.com/yewstack/yew/issues/new?assignees=&labels=bug&template=bug_report.md&title=", vec![text("请在 GitHub 反馈")]),
                    text("。"),
                ]),
            ],
        ),
