...
impl Component for List {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        h3(vec![text("类型化子组件")]),
        p(vec![
            text("在您希望将一种类型的组件作为子组件传递给您的组件的情况下，您可以使用 "),
...
html! {
    <List>
        ...
        p(vec![
            ...
            text(" 来提供更好的人机工程学。如果您不想使用它，您可以为每个变体手动实现 "),
            code("From"),
            text("。"),
        ]),
        code_block(
...
impl Component for List {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        h3(vec![text("可选类型的子组件")]),
        p(vec![text("您还可以具有特定类型的单个可选子组件：")]),
        code_block(
            "rust",
            r#"use yew::{
