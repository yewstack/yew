pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        p(vec![
            ...
            code("Rc::ptr_eq"),
            text(" 的"),
            link("https://doc.rust-lang.org/stable/std/rc/struct.Rc.html#method.ptr_eq", vec![text("更多细节")]),
            text("。"),
        ]),
        p(vec![
            text("这种优化对于不实现 "),
        ...
        h2(vec![text("减小二进制文件大小")]),
        ul(vec![
            li(vec![text("优化 Rust 代码")]),
            li(vec![code("cargo.toml"), text("（定义发布配置文件）")]),
            li(vec![text("使用 "), code("wasm-opt"), text(" 优化 wasm 代码")]),
        ]),
        p(vec![bold(vec![
            text("注意：有关减小二进制文件大小的更多信息，请参阅"),
            link(
                "https://rustwasm.github.io/book/reference/code-size.html#optimizing-builds-for-code-size",
                vec![text("Rust Wasm 手册")],
            ),
            text("。"),
        ])]),
...
lto = true"#),
        ...
        p(vec![
            ...
            link("https://doc.rust-lang.org/cargo/reference/unstable.html", vec![text("不稳定功能")]),
            text("的文档，特别是关于"),
...
build-std-features = ["panic_immediate_abort"]"#),
        admonition(
            ...
            vec![
                p(vec![
                    ...
                    link("https://github.com/yewstack/yew/issues/2696", vec![text("这个例子")]),
        ...
        p(vec![
            ...
            link("https://rustwasm.github.io/book/game-of-life/code-size.html", vec![text("缩小 .wasm 大小")]),
        ]),
        ul(vec![
            li(vec![
                text("使用 "),
        ...
        h4(vec![text("在 yew/examples/ 中 'minimal' 示例的构建大小")]),
        p(vec![
            text("注意："),
            code("wasm-pack"),
        ...
        ul(vec![
            li(vec![
                link("https://doc.rust-lang.org/book/ch15-00-smart-pointers.html", vec![text("Rust 手册中关于智能指针的章节")]),
            ]),
            li(vec![
                link("https://rustwasm.github.io/book/reference/code-size.html#optimizing-builds-for-code-size", vec![text("Rust Wasm 手册中关于减小二进制文件大小的信息")]),
            ]),
            li(vec![
                link("https://doc.rust-lang.org/cargo/reference/profiles.html", vec![text("Rust 配置文件的文档")]),
            ]),
            li(vec![
                link("https://github.com/WebAssembly/binaryen", vec![text("binaryen 项目")]),
            ]),
        ]),
    ])
}
