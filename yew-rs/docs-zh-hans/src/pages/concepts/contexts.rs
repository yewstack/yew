pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        p!["通常，数据是通过 props 从父组件传递到子组件。 \
             但是，如果必须通过中间的许多组件传递它们，或者如果应用程序中的许多组件需要相同的信息，传递 \
             props 可能会变得冗长和烦人。 \
             上下文解决了这个问题，允许父组件使数据可用于其下方树中的任何组件，无论多深，而无需通过 \
             props 传递它们。"],
        h2!["使用 props 的问题：\"Prop Drilling\""],
        p![
            "传递 ",
            link!("/zh-Hans/docs/concepts/function-components/properties", "props"),
            " 是从父组件直接传递数据到子组件的好方法。 \
...
pub fn ThemedButton() -> Html {
    ...
    html! {
        ...
        h2!["使用场景"],
        p!["通常，如果某些数据需要在树的不同部分的远程组件中使用，上下文可能会对你有所帮助。 \
             以下是一些这样的例子："],
        ul![
            li![
                bold!["主题"],
        ...
        h3!["使用上下文前的考虑"],
        p!["上下文非常容易使用，这也使得它们非常容易被误用/过度使用。 \
             仅仅因为你可以使用上下文将 props \
             共享给多个层级深的组件，并不意味着你应该这样做。"],
        p![
            "例如，你可以提取一个组件并将该组件作为子组件传递给另一个组件。例如， 你可能有一个 ",
            code("Layout"),
            " 组件，它将 ",
            code("articles"),
            " 作为 prop 并传递给 ",
            ...
            " 组件。 你应该重构 ",
            code("Layout"),
            " 组件，使其接受子组件作为 props 并显示 ",
            code("<Layout> <ArticleList {articles} /> </Layout>"),
            "。",
        ],
        h2!["修改子组件的上下文值"],
        p![
            "由于 Rust 的所有权规则，上下文不能有一个可以被子组件调用的 ",
            code("&mut self"),
            " 方法。 要修改上下文的值，我们必须将其与 reducer 结合使用。这可以通过使用 ",
            link!(
