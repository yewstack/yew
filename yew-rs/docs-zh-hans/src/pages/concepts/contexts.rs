pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        h2(vec![text("使用 props 的问题：\"Prop Drilling\"")]),
        p(vec![
            text("传递 "),
            link("/zh-Hans/docs/concepts/function-components/properties", vec![text("props")]),
            text(
                " 是从父组件直接传递数据到子组件的好方法。 \
        ...
        p(vec![text("考虑以下示例，它通过 props 传递主题：")]),
        code_block(
            "rust",
    ...
    html! {
        ...
        p(vec![
            ...
            link(
                ...
                vec![text("use_context 文档")],
            ),
            text(" 了解更多信息。"),
        ]),
        h4(vec![text("结构体组件")]),
        p(vec![text("我们有两种选择在结构体组件中使用上下文：")]),
        ul(vec![
            li(vec![
        ...
        ul(vec![
            ...
            li(vec![
                bold(vec![text("当前用户帐户")]),
                text(
                    "：在许多情况下，组件需要知道当前登录的用户。你可以使用上下文将当前用户对象提供给组件。",
                ),
            ]),
        ]),
        h3(vec![text("使用上下文前的考虑")]),
        p(vec![text(
