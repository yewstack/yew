pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        admonition!(
            AdmonitionType::Note,
            None,
            p!["属性 (Properties) 通常被简写为 \"Props\"。"],
        ),
        p!["属性 (Properties) 是组件的参数，Yew 可以监视这些参数。"],
        p![
            "在组件的属性中使用一个类型之前，它必须实现 ",
            code("Properties"),
            " trait。",
        ],
        h2!["响应性"],
        p!["在重新渲染时，Yew 在协调虚拟 DOM \
...
pub struct Props {
    pub is_loading: bool,
        ...
        p![
            "属性 ",
            code("#[component]"),
            " 允许在函数参数中选择性地接收 Props。要提供它们，可以通过 ",
            code("html!"),
            " 宏中的属性进行赋值。",
        ],
        tabs!(
...
fn App() -> Html {
    html! { <HelloWorld /> }
        ...
        p!["以下属性允许您为属性提供默认值，当父组件没有设置它们时将使用这些默认值。"],
        admonition!(
            AdmonitionType::Tip,
            None,
            p!["属性在 Rustdoc \
...
fn Case2() -> Html {
    html! { <Hello name="Sam" /> }
        ...
        p![
            "该宏使用与结构表达式相同的语法，只是您不能使用属性或基本表达式 (",
            code("Foo { ..base }"),
            ")。类型路径可以直接指向属性 (",
            code("path::to::Props"),
            ")，也可以指向组件的关联属性 (",
...
fn main() {
    ...
    assert_eq!(props.last, 3);
        ...
        h2!["反模式"],
        p!["虽然几乎任何 Rust 类型都可以作为属性传递，但有一些反模式应该避免。这些包括但不限于："],
        ol![
            li_blocks![
                p![
                    "使用 ",
