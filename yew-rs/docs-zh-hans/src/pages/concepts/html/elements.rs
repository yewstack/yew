...
fn MyComponent() -> Html {
    ...
    (*node).clone()
        ...
        h2!["动态标签名"],
        p![
            "在构建高阶组件时，您可能会发现自己处于一个标签名不是静态的情况。例如，您可能有一个 ",
            code("Title"),
            " 组件，根据级别属性可以渲染从 ",
            code("h1"),
            " 到 ",
            code("h6"),
            " 的任何内容。而不是使用一个大的匹配表达式，Yew 允许您动态设置标签名，使用 ",
            code("@{name}"),
            "，其中 ",
            code("name"),
            " 可以是返回字符串的任何表达式。",
...
html! {
    <div hidden=true>
        ...
        p!["这与以下的 ", bold!["HTML"], " 功能上是等价的：",],
        code_block("html", r#"<div hidden>This div is hidden.</div>"#),
        p!["将逻辑值属性设置为 false 等效于不使用该属性；可以使用逻辑表达式的值："],
        code_block(
            "rust",
            r#"use yew::prelude::*;
...
html! {
    <div>
        ...
        p![
            "它们都是有效的，",
            bold!["但"],
            "我们鼓励您更倾向于使用 Yew 的自定义 ",
            code("AttrValue"),
            "，特别是如果您需要克隆或将它们作为属性传递给另一个组件。",
        ],
        h2!["HTML 元素的可选属性"],
        p!["大多数 HTML 属性可以使用可选值（Some(x) 或 \
