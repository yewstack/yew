pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        p(vec![
            ...
            link("/zh-Hans/docs/concepts/function-components/callbacks", vec![code("Callback")]),
            text("，请参见"),
            link("#manual-event-listener", vec![text("手动事件监听器")]),
            text("。"),
        ]),
        h2(vec![text("事件类型")]),
        admonition(
...
html! {
    ...
    //      ^^^^^^^ event listener name
        ...
        p(vec![
            ...
            code("click"),
            text(" 事件。查看本页末尾的"),
            link("#available-events", vec![text("完整事件列表")]),
            text("及其类型。"),
        ]),
        h2_id("event-bubbling", vec![text("事件捕获")]),
        p(vec![
            text("Yew 调度的事件遵循虚拟 DOM 层次结构，向上冒泡到监听器。目前，仅支持监听器的冒泡阶段。请注意，虚拟 DOM 层次结构通常（但并非总是）与实际 DOM 层次结构相同。在处理"),
            link("/zh-Hans/docs/advanced-topics/portals", vec![text("传送门")]),
        ...
        ul(vec![
            li(vec![
                ...
                code("HtmlElement"),
                text("，请使用 "),
                link("/zh-Hans/docs/concepts/function-components/node-refs", vec![code("NodeRef")]),
                text("。"),
            ]),
            li(vec![
                link(
        ...
        p(vec![
            ...
            link("/zh-Hans/docs/concepts/html/components#update", vec![text("更新")]),
            text(" "),
            code("Msg"),
            text("。"),
        ]),
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p(vec![
            ...
            code("html"),
            text(" 宏不支持的事件，查看"),
            link("#event-types", vec![text("这里列出的支持的事件")]),
            text("。"),
        ]),
        p(vec![
        ...
        h3(vec![text("使用 "), code("Closure"), text("（冗长版本）")]),
        p(vec![
            text("直接使用 "),
            code("web-sys"),
            text(" 和 "),
            ...
            link(
                "#using-gloo-concise",
                vec![text("感谢 "), code("gloo"), text("，有了更简洁的方法")],
            ),
            text("）。"),
        ]),
        code_block(
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        h3(vec![text("使用 "), code("gloo"), text("（简洁版本）")]),
        p(vec![
            text("更方便的方法是使用 "),
            code("gloo"),
            text("，更具体地说是 "),
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        h2_id("available-events", vec![text("可用事件的完整列表")]),
        table(
