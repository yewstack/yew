pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        h2(vec![text("迭代器")]),
        p(vec![text("从迭代器构建 HTML 有 3 种方法：")]),
        tabs(
            "`for` 循环",
            vec![
                tab(
                    ...
                    vec![
                        p(vec![text(
                            "主要方法是使用 for 循环，与 Rust 中已有的 for 循环相同，但有 2 \
                             个关键区别：",
                        )]),
                        ol(vec![
                            li(vec![
...
html! {
    <div id="introductions">
        ...
        ol(vec![
            li(vec![
                text("进入"),
                link("https://examples.yew.rs/keyed_list", vec![text("在线演示")]),
            ]),
            li(vec![text("添加 500 个元素")]),
            li(vec![text("禁用键")]),
            li(vec![text("反转列表")]),
            li(vec![text(
