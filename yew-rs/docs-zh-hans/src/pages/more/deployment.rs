pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        h2(vec![text("服务器配置")]),
        h3(vec![text("将 "), code("index.html"), text(" 作为回退提供")]),
        p(vec![
            text("如果应用程序使用了 "),
            link("/zh-Hans/docs/concepts/router", vec![text("Yew 路由")]),
        ...
        h3(vec![text("为 Web Assembly 资源配置正确的 MIME 类型。")]),
        p(vec![
            text("WASM 文件必须使用 "),
