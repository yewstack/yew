crate::doc_page!(
    ...
    Content::new(vec![
        ...
        p!["You can still add a ", link!("/docs/0.21/concepts/function-components/callbacks", code("Callback")), " for an event that is not listed in the table below, see ", link!("#manual-event-listener", "Manual event listener"), "."],
        h2!["Event Types"],
...
html! {
    ...
    //      ^^^^^^^ event listener name
        ...
        h2!["Event bubbling"],
        p!["Events dispatched by Yew follow the virtual DOM hierarchy when bubbling up to listeners. Currently, only the bubbling phase is supported for listeners. Note that the virtual DOM hierarchy is most often, but not always, identical to the actual DOM hierarchy. The distinction is important when working with ", link!("/docs/0.21/advanced-topics/portals", "portals"), " and other more advanced techniques. The intuition for well-implemented components should be that events bubble from children to parents. In this way the hierarchy in your coded ", code("html!"), " is the one observed by event handlers."],
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p![link!("/docs/0.21/concepts/function-components/node-refs", code("NodeRef")), " can be used instead of querying the event given to a ", code("Callback"), "."],
        code_block("rust", r#"use web_sys::HtmlInputElement;
use yew::prelude::*;
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p!["In order to add an event listener to one of elements manually we need the help of ", link!("/docs/0.21/concepts/function-components/node-refs", code("NodeRef")), " so that in ", code("use_effect_with"), " we can add a listener using the ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/index.html", code("web-sys")), " and ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/index.html", "wasm-bindgen"), " API."],
