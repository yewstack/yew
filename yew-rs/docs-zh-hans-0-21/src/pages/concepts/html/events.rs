crate::doc_page!(
    ...
    Content::new(vec![
        h2!["Introduction"],
        p!["Yew integrates with the ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/", code("web-sys")), " crate and uses the events from that crate. The ", link!("#event-types", "table below"), " lists all of the ", code("web-sys"), " events that are accepted in the ", code("html!"), " macro."],
        p!["You can still add a ", doc_link!(crate::pages::concepts::function_components::callbacks, code("Callback")), " for an event that is not listed in the table below, see ", link!("#manual-event-listener", "Manual event listener"), "."],
        h2!["Event Types"],
        admonition!(AdmonitionType::Tip, None, p!["All the event types mentioned in the following table are re-exported under ", code("yew::events"), ". Using the types from ", code("yew::events"), " makes it easier to ensure version compatibility than if you were to manually include ", code("web-sys"), " as a dependency in your crate because you will not end up using a version which conflicts with the version that Yew specifies."]),
        p!["The event listener name is the expected name when adding an event ", code("Callback"), " in the ", code("html"), " macro:"],
...
html! {
    ...
    //      ^^^^^^^ event listener name
        ...
        p!["The event name is the listener without the \"on\" prefix, therefore, the ", code("onclick"), " event listener listens for ", code("click"), " events. See the end of this page for a ", link!("#available-events", "full list of available events"), " with their types."],
        h2!["Event bubbling"],
        p!["Events dispatched by Yew follow the virtual DOM hierarchy when bubbling up to listeners. Currently, only the bubbling phase is supported for listeners. Note that the virtual DOM hierarchy is most often, but not always, identical to the actual DOM hierarchy. The distinction is important when working with ", doc_link!(crate::pages::advanced_topics::portals, "portals"), " and other more advanced techniques. The intuition for well-implemented components should be that events bubble from children to parents. In this way the hierarchy in your coded ", code("html!"), " is the one observed by event handlers."],
        p!["If you are not interested in event bubbling, you can turn it off by calling"],
        code_block("rust", r#"yew::set_event_bubbling(false);"#),
        p![italic!["before"], " starting your app. This speeds up event handling, but some components may break from not receiving the events they expect. Use this with care!"],
        h2!["Event delegation"],
        p!["It can be surprising that event listeners are ", italic!["not"], " directly registered on the element where they are rendered. Instead, events are delegated from the subtree root of the Yew app. Still, events are delivered in their native form, and no synthetic form is created. This can lead to mismatches between the event you would expect in HTML listeners and those showing up in Yew."],
        ul![li![link!("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.Event.html#method.current_target", code("Event::current_target")), " points to the Yew subtree root instead of the element the listener is added on. Use ", doc_link!(crate::pages::concepts::function_components::node_refs, code("NodeRef")), " if you want access to the underlying ", code("HtmlElement"), "."], li![link!("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.Event.html#method.event_phase", code("Event::event_phase")), " is always ", code("Event::CAPTURING_PHASE"), ". Internally, the event will behave as if it was in the bubbling phase, the event propagation is replayed and the event ", link!("#event-bubbling", "bubbles up"), ", i.e. event listeners higher up in the virtual DOM will trigger after event listeners below them. Currently, capturing listeners is not supported by Yew. This also means that events registered by Yew will usually fire before other event listeners."]],
        h2!["Typed event target"],
        admonition!(AdmonitionType::Caution, None, p!["In this section ", bold!["target"], " (", link!("https://developer.mozilla.org/en-US/docs/Web/API/Event/target", code("Event.target")), ") is always referring to the element at which the event was dispatched from."], p!["This will ", bold!["not"], " always be the element at which the ", code("Callback"), " is placed."]),
        p!["In event ", code("Callback"), "s you may want to get the target of that event. For example, the ", code("change"), " event gives no information but is used to notify that something has changed."],
        p!["In Yew getting the target element in the correct type can be done in a few ways and we will go through them here. Calling ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.Event.html#method.target", code("web_sys::Event::target")), " on an event returns an optional ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.EventTarget.html", code("web_sys::EventTarget")), " type, which might not seem very useful when you want to know the value of your input element."],
        p!["In all the approaches below we are going to tackle the same problem, so it is clear where the approach \
          differs as opposed to the problem at hand."],
        p![bold!["The Problem:"]],
        p!["We have an ", code("onchange"), " ", code("Callback"), " on my ", code("<input>"), " element and each time it is invoked we want to send an update ", code("Msg"), " to our component."],
        p!["Our ", code("Msg"), " enum looks like this:"],
        code_block("rust", r#"pub enum Msg {
    InputValue(String),
}"#),
        h3!["Using ", code("JsCast")],
        p!["The ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/index.html", code("wasm-bindgen")), " crate has a useful trait; ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/trait.JsCast.html", code("JsCast")), ", which allows us to hop and skip our way to the type we want, as long as it implements ", code("JsCast"), ". We can do this cautiously, which involves some runtime checks and failure types like ", code("Option"), " and ", code("Result"), ", or we can do it dangerously."],
        p!["Enough talk, more code:"],
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p!["The methods from ", code("JsCast"), " are ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/trait.JsCast.html#method.dyn_into", code("dyn_into")), " and ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/trait.JsCast.html#method.unchecked_into", code("unchecked_into")), " and they allowed us to go from ", code("EventTarget"), " to ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.HtmlInputElement.html", code("HtmlInputElement")), ". The ", code("dyn_into"), " method is cautious because at runtime it will check whether the type is actually a ", code("HtmlInputElement"), " and if not return an ", code("Err(JsValue)"), ", the ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/struct.JsValue.html", code("JsValue")), " is a catch-all type and is essentially giving you back the object to try again."],
        p!["At this point you might be thinking... when is the dangerous version ok to use? In the case above it is safe", sup!["1"], " as we've set the ", code("Callback"), " on to an element with no children so the target can only be that same element."],
        p![italic![sup!["1"], " As safe as anything can be when JS land is involved."]],
        h3!["Using ", code("TargetCast")],
        p![bold!["It is highly recommended to read "], link!("#using-jscast", "Using JsCast"), bold![" first!"]],
        admonition!(AdmonitionType::Note, None, p![code("TargetCast"), " was designed to feel very similar to ", code("JsCast"), " - this is to allow new users to get a feel for the behaviour of ", code("JsCast"), " but with the smaller scope of events and their targets."], p![code("TargetCast"), " vs ", code("JsCast"), " is purely preference, you will find that ", code("TargetCast"), " implements something similar to what you would using ", code("JsCast"), "."]),
        p!["The ", code("TargetCast"), " trait is built on top of ", code("JsCast"), " and is specialized towards getting typed event targets from events."],
        p![code("TargetCast"), " comes with Yew so no need to add a dependency in order to use the trait methods on events but it works in a very similar way to ", code("JsCast"), "."],
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p!["If you followed the advice above and read about ", code("JsCast"), ", you can probably see that ", code("TargetCast::target_dyn_into"), " feels similar to ", code("JsCast::dyn_into"), " but specifically does the cast on the target of the event. ", code("TargetCast::target_unchecked_into"), " is similar to ", code("JsCast::unchecked_into"), ", and as such all the same warnings above ", code("JsCast"), " apply to ", code("TargetCast"), "."],
        h3!["Using ", code("NodeRef")],
        p![doc_link!(crate::pages::concepts::function_components::node_refs, code("NodeRef")), " can be used instead of querying the event given to a ", code("Callback"), "."],
        code_block("rust", r#"use web_sys::HtmlInputElement;
use yew::prelude::*;
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p!["Using ", code("NodeRef"), ", you can ignore the event and use the ", code("NodeRef::cast"), " method to get an ", code("Option<HtmlInputElement>"), " - this is optional as calling ", code("cast"), " before the ", code("NodeRef"), " has been set, or when the type doesn't match will return ", code("None"), "."],
        p!["You might also see by using ", code("NodeRef"), " we don't have to send the ", code("String"), " back into state as we always have access to ", code("input_node_ref"), " - so we could do the following:"],
        code_block("rust", r#"use web_sys::HtmlInputElement;
use yew::prelude::*;
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p!["Which approach you take depends on your component and your preferences, there is no ", italic!["blessed"], " way per se."],
        h2!["Manual event listener"],
        p!["You may want to listen to an event that is not supported by Yew's ", code("html"), " macro, see the ", link!("#event-types", "supported events listed here"), "."],
        p!["In order to add an event listener to one of elements manually we need the help of ", doc_link!(crate::pages::concepts::function_components::node_refs, code("NodeRef")), " so that in ", code("use_effect_with"), " we can add a listener using the ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/index.html", code("web-sys")), " and ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/index.html", "wasm-bindgen"), " API."],
        p!["The examples below are going to show adding listeners for the made-up ", code("custard"), " event. All events either unsupported by yew or custom can be represented as a ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.Event.html", code("web_sys::Event")), ". If you need to access a specific method or field on a custom / unsupported event then you can use the methods of ", link!("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/trait.JsCast.html", code("JsCast")), " in order to convert to the type required."],
        h3!["Using ", code("Closure"), " (verbose)"],
        p!["Using the ", code("web-sys"), " and ", code("wasm-bindgen"), " API's directly for this can be a bit painful.. so brace yourself (", link!("#using-gloo-concise", "there is a more concise way thanks to ", code("gloo")), ")."],
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p!["For more information on ", code("Closures"), ", see ", link!("https://wasm-bindgen.github.io/wasm-bindgen/examples/closures.html", "The wasm-bindgen Guide"), "."],
        h3!["Using ", code("gloo"), " (concise)"],
        p!["The easier way is with ", code("gloo"), ", more specifically ", link!("https://docs.rs/gloo-events/0.1.1/gloo_events/index.html", code("gloo_events")), " which is an abstraction for ", code("web-sys"), ", ", code("wasm-bindgen"), "."],
        p![code("gloo_events"), " has the ", code("EventListener"), " type which can be used to create and store the event listener."],
        code_block("toml", r#"[dependencies]
gloo-events = "0.1""#),
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p!["For more information on ", code("EventListener"), ", see the ", link!("https://docs.rs/gloo-events/0.1.1/gloo_events/struct.EventListener.html", "gloo_events docs.rs"), "."],
        h2!["Full list of available events"],
        table(
            vec![
