crate::doc_page!(
    ...
    Content::new(vec![
        ...
        p(vec![text("Yew integrates with the "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/", vec![code("web-sys")]), text(" crate and uses the events from that crate. The "), link("#event-types", vec![text("table below")]), text(" lists all of the "), code("web-sys"), text(" events that are accepted in the "), code("html!"), text(" macro.")]),
        p(vec![text("You can still add a "), link("/docs/0.21/concepts/function-components/callbacks", vec![code("Callback")]), text(" for an event that is not listed in the table below, see "), link("#manual-event-listener", vec![text("Manual event listener")]), text(".")]),
        h2(vec![text("Event Types")]),
        admonition(AdmonitionType::Tip, None, vec![p(vec![text("All the event types mentioned in the following table are re-exported under "), code("yew::events"), text(". Using the types from "), code("yew::events"), text(" makes it easier to ensure version compatibility than if you were to manually include "), code("web-sys"), text(" as a dependency in your crate because you will not end up using a version which conflicts with the version that Yew specifies.")])]),
        p(vec![text("The event listener name is the expected name when adding an event "), code("Callback"), text(" in the "), code("html"), text(" macro:")]),
...
html! {
    ...
    //      ^^^^^^^ event listener name
        ...
        p(vec![text("The event name is the listener without the \"on\" prefix, therefore, the "), code("onclick"), text(" event listener listens for "), code("click"), text(" events. See the end of this page for a "), link("#available-events", vec![text("full list of available events")]), text(" with their types.")]),
        h2(vec![text("Event bubbling")]),
        p(vec![text("Events dispatched by Yew follow the virtual DOM hierarchy when bubbling up to listeners. Currently, only the bubbling phase is supported for listeners. Note that the virtual DOM hierarchy is most often, but not always, identical to the actual DOM hierarchy. The distinction is important when working with "), link("/docs/0.21/advanced-topics/portals", vec![text("portals")]), text(" and other more advanced techniques. The intuition for well-implemented components should be that events bubble from children to parents. In this way the hierarchy in your coded "), code("html!"), text(" is the one observed by event handlers.")]),
        p(vec![text("If you are not interested in event bubbling, you can turn it off by calling")]),
        code_block("rust", r#"yew::set_event_bubbling(false);"#),
        p(vec![italic(vec![text("before")]), text(" starting your app. This speeds up event handling, but some components may break from not receiving the events they expect. Use this with care!")]),
        h2(vec![text("Event delegation")]),
        p(vec![text("It can be surprising that event listeners are "), italic(vec![text("not")]), text(" directly registered on the element where they are rendered. Instead, events are delegated from the subtree root of the Yew app. Still, events are delivered in their native form, and no synthetic form is created. This can lead to mismatches between the event you would expect in HTML listeners and those showing up in Yew.")]),
        ul(vec![li(vec![link("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.Event.html#method.current_target", vec![code("Event::current_target")]), text(" points to the Yew subtree root instead of the element the listener is added on. Use "), link("/docs/0.21/concepts/function-components/node-refs", vec![code("NodeRef")]), text(" if you want access to the underlying "), code("HtmlElement"), text(".")]), li(vec![link("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.Event.html#method.event_phase", vec![code("Event::event_phase")]), text(" is always "), code("Event::CAPTURING_PHASE"), text(". Internally, the event will behave as if it was in the bubbling phase, the event propagation is replayed and the event "), link("#event-bubbling", vec![text("bubbles up")]), text(", i.e. event listeners higher up in the virtual DOM will trigger after event listeners below them. Currently, capturing listeners is not supported by Yew. This also means that events registered by Yew will usually fire before other event listeners.")])]),
        h2(vec![text("Typed event target")]),
        admonition(AdmonitionType::Caution, None, vec![p(vec![text("In this section "), bold(vec![text("target")]), text(" ("), link("https://developer.mozilla.org/en-US/docs/Web/API/Event/target", vec![code("Event.target")]), text(") is always referring to the element at which the event was dispatched from.")]), p(vec![text("This will "), bold(vec![text("not")]), text(" always be the element at which the "), code("Callback"), text(" is placed.")])]),
        p(vec![text("In event "), code("Callback"), text("s you may want to get the target of that event. For example, the "), code("change"), text(" event gives no information but is used to notify that something has changed.")]),
        p(vec![text("In Yew getting the target element in the correct type can be done in a few ways and we will go through them here. Calling "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.Event.html#method.target", vec![code("web_sys::Event::target")]), text(" on an event returns an optional "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.EventTarget.html", vec![code("web_sys::EventTarget")]), text(" type, which might not seem very useful when you want to know the value of your input element.")]),
        p(vec![text("In all the approaches below we are going to tackle the same problem, so it is clear where the approach \
          differs as opposed to the problem at hand.")]),
        p(vec![bold(vec![text("The Problem:")])]),
        p(vec![text("We have an "), code("onchange"), text(" "), code("Callback"), text(" on my "), code("<input>"), text(" element and each time it is invoked we want to send an update "), code("Msg"), text(" to our component.")]),
        p(vec![text("Our "), code("Msg"), text(" enum looks like this:")]),
        code_block("rust", r#"pub enum Msg {
    InputValue(String),
}"#),
        h3(vec![text("Using "), code("JsCast")]),
        p(vec![text("The "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/index.html", vec![code("wasm-bindgen")]), text(" crate has a useful trait; "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/trait.JsCast.html", vec![code("JsCast")]), text(", which allows us to hop and skip our way to the type we want, as long as it implements "), code("JsCast"), text(". We can do this cautiously, which involves some runtime checks and failure types like "), code("Option"), text(" and "), code("Result"), text(", or we can do it dangerously.")]),
        p(vec![text("Enough talk, more code:")]),
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p(vec![text("The methods from "), code("JsCast"), text(" are "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/trait.JsCast.html#method.dyn_into", vec![code("dyn_into")]), text(" and "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/trait.JsCast.html#method.unchecked_into", vec![code("unchecked_into")]), text(" and they allowed us to go from "), code("EventTarget"), text(" to "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.HtmlInputElement.html", vec![code("HtmlInputElement")]), text(". The "), code("dyn_into"), text(" method is cautious because at runtime it will check whether the type is actually a "), code("HtmlInputElement"), text(" and if not return an "), code("Err(JsValue)"), text(", the "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/struct.JsValue.html", vec![code("JsValue")]), text(" is a catch-all type and is essentially giving you back the object to try again.")]),
        p(vec![text("At this point you might be thinking... when is the dangerous version ok to use? In the case above it is safe"), sup(vec![text("1")]), text(" as we've set the "), code("Callback"), text(" on to an element with no children so the target can only be that same element.")]),
        p(vec![italic(vec![sup(vec![text("1")]), text(" As safe as anything can be when JS land is involved.")])]),
        h3(vec![text("Using "), code("TargetCast")]),
        p(vec![bold(vec![text("It is highly recommended to read ")]), link("#using-jscast", vec![text("Using JsCast")]), bold(vec![text(" first!")])]),
        admonition(AdmonitionType::Note, None, vec![p(vec![code("TargetCast"), text(" was designed to feel very similar to "), code("JsCast"), text(" - this is to allow new users to get a feel for the behaviour of "), code("JsCast"), text(" but with the smaller scope of events and their targets.")]), p(vec![code("TargetCast"), text(" vs "), code("JsCast"), text(" is purely preference, you will find that "), code("TargetCast"), text(" implements something similar to what you would using "), code("JsCast"), text(".")])]),
        p(vec![text("The "), code("TargetCast"), text(" trait is built on top of "), code("JsCast"), text(" and is specialized towards getting typed event targets from events.")]),
        p(vec![code("TargetCast"), text(" comes with Yew so no need to add a dependency in order to use the trait methods on events but it works in a very similar way to "), code("JsCast"), text(".")]),
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p(vec![text("If you followed the advice above and read about "), code("JsCast"), text(", you can probably see that "), code("TargetCast::target_dyn_into"), text(" feels similar to "), code("JsCast::dyn_into"), text(" but specifically does the cast on the target of the event. "), code("TargetCast::target_unchecked_into"), text(" is similar to "), code("JsCast::unchecked_into"), text(", and as such all the same warnings above "), code("JsCast"), text(" apply to "), code("TargetCast"), text(".")]),
        h3(vec![text("Using "), code("NodeRef")]),
        p(vec![link("/docs/0.21/concepts/function-components/node-refs", vec![code("NodeRef")]), text(" can be used instead of querying the event given to a "), code("Callback"), text(".")]),
        code_block("rust", r#"use web_sys::HtmlInputElement;
use yew::prelude::*;
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p(vec![text("Using "), code("NodeRef"), text(", you can ignore the event and use the "), code("NodeRef::cast"), text(" method to get an "), code("Option<HtmlInputElement>"), text(" - this is optional as calling "), code("cast"), text(" before the "), code("NodeRef"), text(" has been set, or when the type doesn't match will return "), code("None"), text(".")]),
        p(vec![text("You might also see by using "), code("NodeRef"), text(" we don't have to send the "), code("String"), text(" back into state as we always have access to "), code("input_node_ref"), text(" - so we could do the following:")]),
        code_block("rust", r#"use web_sys::HtmlInputElement;
use yew::prelude::*;
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p(vec![text("Which approach you take depends on your component and your preferences, there is no "), italic(vec![text("blessed")]), text(" way per se.")]),
        h2(vec![text("Manual event listener")]),
        p(vec![text("You may want to listen to an event that is not supported by Yew's "), code("html"), text(" macro, see the "), link("#event-types", vec![text("supported events listed here")]), text(".")]),
        p(vec![text("In order to add an event listener to one of elements manually we need the help of "), link("/docs/0.21/concepts/function-components/node-refs", vec![code("NodeRef")]), text(" so that in "), code("use_effect_with"), text(" we can add a listener using the "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/index.html", vec![code("web-sys")]), text(" and "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/index.html", vec![text("wasm-bindgen")]), text(" API.")]),
        p(vec![text("The examples below are going to show adding listeners for the made-up "), code("custard"), text(" event. All events either unsupported by yew or custom can be represented as a "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/web_sys/struct.Event.html", vec![code("web_sys::Event")]), text(". If you need to access a specific method or field on a custom / unsupported event then you can use the methods of "), link("https://wasm-bindgen.github.io/wasm-bindgen/api/wasm_bindgen/trait.JsCast.html", vec![code("JsCast")]), text(" in order to convert to the type required.")]),
        h3(vec![text("Using "), code("Closure"), text(" (verbose)")]),
        p(vec![text("Using the "), code("web-sys"), text(" and "), code("wasm-bindgen"), text(" API's directly for this can be a bit painful.. so brace yourself ("), link("#using-gloo-concise", vec![text("there is a more concise way thanks to "), code("gloo")]), text(").")]),
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p(vec![text("For more information on "), code("Closures"), text(", see "), link("https://wasm-bindgen.github.io/wasm-bindgen/examples/closures.html", vec![text("The wasm-bindgen Guide")]), text(".")]),
        h3(vec![text("Using "), code("gloo"), text(" (concise)")]),
        p(vec![text("The easier way is with "), code("gloo"), text(", more specifically "), link("https://docs.rs/gloo-events/0.1.1/gloo_events/index.html", vec![code("gloo_events")]), text(" which is an abstraction for "), code("web-sys"), text(", "), code("wasm-bindgen"), text(".")]),
        p(vec![code("gloo_events"), text(" has the "), code("EventListener"), text(" type which can be used to create and store the event listener.")]),
        code_block("toml", r#"[dependencies]
gloo-events = "0.1""#),
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        p(vec![text("For more information on "), code("EventListener"), text(", see the "), link("https://docs.rs/gloo-events/0.1.1/gloo_events/struct.EventListener.html", vec![text("gloo_events docs.rs")]), text(".")]),
        h2(vec![text("Full list of available events")]),
        table(
            vec![
