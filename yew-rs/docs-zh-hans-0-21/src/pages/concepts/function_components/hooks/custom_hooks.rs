crate::doc_page!(
    ...
    Content::new(vec![
        h2!["定义自定义钩子"],
        p!["组件中与状态有关的逻辑可以通过创建自定义 Hooks 提取到函数中。"],
        p!["假设我们有一个组件，它订阅了一个代理（agent）并且会显示发送给它的消息。"],
        code_block(
            "rust",
...
pub fn show_messages() -> Html {
    ...
    html! { <div>{ for output }</div> }
        ...
        p![
            ...
            "的新函数。 ",
            code("use_"),
            "前缀通常表示此函数是一个钩子。这个函数将不接受任何参数并返回",
    ...
    // ...
        ...
        p![
            ...
            "它们两个的内部状态的可变引用。 而",
            code("hook_callback"),
            "同样也要 2 个参数：一个回调和一个 bool，回调接受",
            code("internal_state"),
            " ，也就是对内部状态实例的可变引用，并且会调执行实际的更改，还会返回表示",
            code("ShouldRender"),
            "的布尔值，第二个参数 bool 的用处是指示它是否在组件渲染后运行。",
            code("use_hook"),
            "的第二个参数",
            code("initial_state_producer"),
            "接受用于创建内部状态实例的回调。这里说的内部状态指的是一个实现了",
            code("Hook"),
            " trait 的结构体。"
        ],
        p![
...
impl Hook for UseSubscribeState {}"#
        ...
        p!["接下来我们为", code("use_subscribe"), "添加实际逻辑。"],
        code_block(
            "rust",
            r#"fn use_subscribe() -> Rc<RefCell<Vec<String>>> {
    use_hook(
