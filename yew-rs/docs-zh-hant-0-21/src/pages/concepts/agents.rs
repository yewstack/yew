crate::doc_page!(
    ...
    Content::new(vec![
        h1(vec![text("Agents")]),
        p(vec![
            text("Agents 類似於 Angular 的 "),
            link(
                "https://angular.io/guide/architecture-services",
                vec![text("Services")],
            ),
            text("（但沒有依賴注入）而且提供 Yew 一個 "),
            link(
                "https://en.wikipedia.org/wiki/Actor_model",
                vec![text("Actor Model")],
            ),
            text(
                "。Agents 可以用來作為兩個元件間的路由訊息，而且與他們在元件間的層級關係獨立出來，\
                 所以他也可以用來作為一個全域的狀態，甚至可以用來減輕用來渲染 UI \
                 畫面的主執行緒的大量運算任務。 未來，我們還規劃要讓 agents 幫忙 Yew 專案可以跨頁籤溝通。",
            ),
        ]),
        p(vec![
        ...
        ul(vec![
            ...
            li(vec![text(
                "Context - Bridges 會建立並連接上 UI 執行緒上的 agent。\
                 這可以用來協調元件與其他 agent 之間的狀態。當沒有任何 bridge 連接上這個 agent，\
                 這個 agent 就會消失。",
            )]),
            li(vec![text(
            ...
            li(vec![text("Global（編寫中）")]),
        ]),
        h2(vec![text("在 Agents 與元件之間溝通")]),
        h3(vec![text("Bridges")]),
        p(vec![text(
        ...
        p(vec![text(
            "dispatcher 允許元件與 agent 進行單向的溝通。dispatcher 也允許元件向 agent 發送訊息。",
        )]),
        h2(vec![text("開銷")]),
        p(vec![
            text("Agents 透過使用 "),
            ...
            text(
                " 序列化他們的訊息，來溝通。所以比起呼叫方法，他的效能花費比較高。\
                 除非計算的成本，或是跨元件計算的成本，比傳遞訊息的成本要高，否則 agent \
                 的方法儘量只有包含單純的邏輯運算。",
            ),
        ]),
        h2(vec![text("延伸閱讀")]),
        ul(vec![li(vec![
            link(
                ...
                vec![text("web_worker_fib")],
            ),
            text(" 範例顯示了如何在 agents 之間溝通。"),
        ])]),
    ])
);
