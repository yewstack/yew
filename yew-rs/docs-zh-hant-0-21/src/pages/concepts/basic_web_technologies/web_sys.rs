...
fn inheritance_of_text_area(text_area: HtmlTextAreaElement) {
    ...
    let event_target: &EventTarget = text_area.as_ref();
        ...
        p!["這一節展示如何將與 Web API 互動的 JavaScript 程式碼範例重寫為 Rust 中的 ", code("web-sys"), "。"],
        h3!["JavaScript 範例"],
        code_block(
            "js",
...
// 我們現在需要保存 `mousemove` 閉包，以便在事件觸發時閉包仍然在記憶體中。"#,
        ...
        p!["這個版本更加冗長，但你可能會注意到其中的一部分是由於失敗類型提醒我們，一些函數呼叫有必須保持的不變量，否則將在 \
             Rust 中引發 panic。另一個冗長的部分是呼叫 ", code("JsCast"), " 來將不同類型轉換為特定類型，以便呼叫其特定方法。"],
        h3!["用 Yew 重寫的範例"],
        p![
            "在Yew 中，您將主要建立 ",
            link!(
