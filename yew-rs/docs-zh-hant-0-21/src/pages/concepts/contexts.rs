crate::doc_page!(
    ...
    Content::new(vec![
        p!["通常，資料是透過 props 從父元件傳遞到子元件。 \
             但是，如果必須透過中間的許多元件傳遞它們，或者如果應用程式中的許多元件需要相同的訊息，傳遞 props \
             可能會變得冗長和煩人。 \
        ...
        p![
            ...
            " 是從父元件直接傳遞資料到子元件的好方法。 \
                 ...
                 然而，這可能導致 props 必須通過多個元件才能到達需要它的元件。 這種情況稱為 \"Prop Drilling\"。",
        ],
        p!["考慮以下範例，它透過 props 傳遞主題："],
        code_block(
            "rust",
            r#"use yew::{html, Component, Context, Html, Properties, function_component};
...
fn App() -> Html {
    ...
    html! {
        ...
        p![
            ...
            " 是其子元件將擁有上下文的元件。 \
                 當上下文變更時，子元件會重新渲染。一個結構體用來定義要傳遞的資料。 ",
            code("ContextProvider"),
            " 可以這樣使用：",
        ],
        code_block(
...
pub fn ThemedButton() -> Html {
    ...
    html! {
        ...
        h2!["使用場景"],
        p!["通常，如果某些資料需要在樹的不同部分的遠端元件中使用，上下文可能會對你有所幫助。 \
             以下是一些這樣的例子："],
        ul![
            li![
                bold!["主題"],
