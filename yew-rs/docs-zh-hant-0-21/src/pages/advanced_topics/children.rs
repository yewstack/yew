crate::doc_page!(
    ...
    Content::new(vec![
        ...
        h2!["一般用法"],
        p![
            "*大多數情況下，*當允許元件具有子元件時，您不關心元件具有的子元件的類型。\
             在這種情況下，下面的範例就足夠了。"
        ],
        code_block(
            "rust",
...
impl Component for List {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        h3!["類型化子元件"],
        p![
            "在您希望將一種類型的元件作為子元件傳遞給您的元件的情況下，您可以使用 ",
...
impl Component for List {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        h2!["帶有屬性的巢狀子元件"],
        p!["如果包含元件對其子元件進行了類型化，則可以存取和變更巢狀元件的屬性。"],
        code_block(
            "rust",
            r#"use std::rc::Rc;
use yew::prelude::*;
...
html! {
    <List>
        ...
        p![
            ...
            " 來提供更好的人體工學。如果您不想使用它，您可以為每個變體手動實現 ",
            code("From"),
            "。",
        ],
        code_block(
...
pub fn render_page(with_sidebar: bool) -> Html {
    if with_sidebar {
        ...
        h2!["進一步閱讀"],
        ul![li![
            "有關此模式的真實範例，請查閱 yew-router 的原始程式碼。有關更高級的範例，請查看 yew \
             儲存庫中的",
            link!(
