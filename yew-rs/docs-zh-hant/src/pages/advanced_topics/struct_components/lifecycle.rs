    ...
    fn view(&self, _ctx: &Context<Self>) -> Html {
        ...
        p![
            ...
            " 方法可讓您描述元件應該如何呈現到 DOM 中。使用Rust 函數編寫類似HTML \
             的程式碼可能會變得非常混亂，因此Yew 提供了一個名為",
            code("html!"),
            " 的宏，用於聲明HTML 和SVG \
             節點（以及將屬性和事件監聽器附加到它們）以及一種方便的方法來渲染子元件。\
             這個宏在某種程度上類似於 React 的 JSX（除了程式語言的差異）。一個不同之處是 Yew \
             提供了一種類似 Svelte 的屬性的簡寫語法，其中您可以只寫 ",
            code("{onclick}"),
            "，而不用寫 ",
...
impl Component for MyComponent {
    ...
    // highlight-end
        ...
        p![
            ...
            " 元件生命週期方法在 ",
            code("view"),
            " 被調用並且 Yew 已經將結果渲染到 DOM \
             中後調用，但在瀏覽器刷新頁面之前。當您想要執行只能在元件渲染元素後完成的操作時，\
             此方法非常有用。還有一個名為 ",
            code("first_render"),
...
impl Component for MyComponent {
    ...
    fn view(&self, _ctx: &Context<Self>) -> Html {
        ...
        p![
            "元件從 DOM 卸載後，Yew 會呼叫 ",
            code("destroy"),
            " 生命週期方法；如果您需要在元件被銷毀之前執行清理操作，這是必要的。此方法是可選的，\
             預設不執行任何操作。",
        ],
        h3!["無限循環"],
        p![
...
impl Component for Comp {
    ...
    fn rendered(&mut self, ctx: &Context<Self>, _first_render: bool) {
        ...
        p!["讓我們看看這裡發生了什麼："],
        ol![
            li!["使用 ", code("create"), " 函數建立元件。"],
            li![
                "呼叫 ",
                code("view"),
                " 方法，以便 Yew 知道要渲染到瀏覽器 DOM 中的內容。",
            ],
            li![
                "呼叫 ",
                code("rendered"),
            ...
            li!["Yew 完成後渲染階段。"],
            li!["Yew 檢查已排程的事件，並看到更新訊息佇列不為空，因此處理訊息。"],
            li![
                "呼叫 ",
                code("update"),
                " 方法，回傳 ",
    ...
    // ...
        ...
        p![
            "在組織程式碼庫時，將 ",
            code("Message"),
            " 類型的定義包含在定義元件的相同模組中是明智的。\
             您可能會發現採用一致的命名約定來命名訊息類型很有幫助。\
             一個選項（儘管不是唯一的選項）是將類型命名為 ",
    ...
    FormInput(String)
        ...
        p![
            code("Properties"),
            " 表示從其父元件傳遞給元件的訊息。此類型必須實作 ",
            code("Properties"),
            " trait（通常透過衍生它）並且可以指定某些屬性是必需的還是可選的。\
             在建立和更新元件時使用此類型。在組件的模組中建立一個名為 ",
            code("Props"),
            " 的結構體，並將其用作組件的 ",
            code("Properties"),
            " 類型是一種常見做法。通常將 \"properties\" 縮寫為 \"props\"。由於 props \
             是從父元件傳遞下來的，因此應用程式的根元件通常具有 ",
            code("Properties"),
            " 類型為 ",
            code("()"),
            "。如果要為根元件指定屬性，請使用 ",
