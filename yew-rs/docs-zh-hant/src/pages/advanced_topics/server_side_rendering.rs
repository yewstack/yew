pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        h1!["服務端渲染 (Server-Side Rendering)"],
        p![
    ...
    println!("{}", rendered);
        ...
        admonition![
            ...
            p![
                "瀏覽器相關的接口，如 ",
                code("web_sys"),
                "，在元件在服務端渲染時是不可用的。如果您嘗試使用它們，您的應用程式將會崩潰。您應該將需要這部分邏輯隔離在 ",
                code("use_effect"),
                " 或 ",
                code("use_effect_with"),
                " 中，因為在服務端渲染時它們無法也不應執行。",
            ],
        ],
        admonition![
            ...
            Some("結構化組件"),
            p![
                "儘管可以在服務端渲染時使用結構化元件，但是在客戶端安全邏輯（如函數元件的",
                code("use_effect"),
                " 鉤子）和生命週期事件之間沒有明確的邊界，並且生命週期事件的呼叫順序與客戶端不同。",
            ],
            p![
                "此外，結構化元件將繼續接受訊息，直到所有子元件都被渲染並呼叫了 ",
                code("destroy"),
                " 方法。開發人員需要確保不會將可能傳遞給元件的訊息連結到呼叫瀏覽器介面的邏輯。",
            ],
            p![
        ...
        p![
            code("<Suspense />"),
            " 是一個特殊的元件，當在客戶端使用時，它提供了一種在元件獲取資料（掛起）時顯示一個回退UI 的方法，並在資料獲取完成後恢復到正常UI。",
        ],
        p![
        ...
        p![
            ...
            "、",
            code("<meta>"),
            "），使爬蟲和社群預覽在首次載入時能看到正確的中繼資料。",
        ],
        p![
            code("ServerRenderer"),
            " 只渲染元件樹（通常對應文件的 body 部分），無法存取 ",
            code("<head>"),
            "。因此，head 標籤必須",
            bold!["在伺服器端、Yew 之外"],
            "產生，並在傳送給客戶端之前拼接到 HTML 範本中。",
        ],
        p![
            link!["https://github.com/yewstack/yew/blob/master/examples/ssr_router/src/bin/ssr_router_server.rs", code("ssr_router"), " 範例"],
            " 展示了這個模式：伺服器從請求 URL 識別路由，產生適當的 ",
            code("<title>"),
            " 和 ",
            code("<meta>"),
            " 標籤，並將它們注入到 Trunk 產生的 ",
            code("index.html"),
            " 的 ",
            code("</head>"),
            " 之前。",
        ...
        admonition![
            ...
            None,
            p![
                "如需完全相容 SSR 的第三方解決方案，請使用 ",
                link!["https://docs.rs/bounce/latest/bounce/helmet/index.html", "Bounce 的 ", code("<Helmet/>"), " 元件"],
                "。",
            ],
        ],
        h2!["SSR 水合（SSR Hydration）"],
        p![
            "水合是將 Yew 應用程式連接到服務端產生的 HTML 檔案的過程。預設情況下，",
            code("ServerRender"),
            " 會列印可水合的 HTML 字串，其中包含額外的資訊以便於水合。當呼叫 ",
            code("Renderer::hydrate"),
            " 方法時，Yew 不會從頭開始渲染，而是將應用程式產生的虛擬 DOM 與伺服器渲染器產生的 HTML 字串進行協調。",
        ],
        admonition![
            AdmonitionType::Caution,
            None,
            p![
                "要成功對由 ",
                code("ServerRenderer"),
                " 建立的 HTML 標記進行水合，用戶端必須產生一個虛擬 DOM 佈局，它與用於 SSR 的佈局完全匹配，包括不包含任何元素的元件。如果您有任何只在一個實作中有用的元件，您可能想要使用 ",
        ...
        admonition![
            ...
            None,
            p![
                "只有在瀏覽器初始渲染 SSR 輸出（靜態 HTML）後，真實 DOM 與預期 DOM 相符時，水合才能成功。如果您的 HTML 不符合規範，水合可能會失敗。瀏覽器可能會更改不正確的 HTML 的 DOM 結構，導致實際 DOM 與預期 DOM 不同。例如，",
                link!["https://github.com/yewstack/yew/issues/2684", "如果您有一個沒有", code("<tbody>"), " 的", code("<table>"), "，瀏覽器可能會向DOM 添加一個", code("<tbody>")],
            ],
        ],
        h2!["水合期間的組件生命週期"],
        p![
            "在水合期間，元件在創建後安排了 2 次連續的渲染。任何效果都是在第二次渲染完成後調用的。確保您的元件的渲染函數沒有副作用是很重要的。它不應該改變任何狀態或觸發額外的渲染。如果您的元件目前改變狀態或觸發額外的渲染，請將它們移到 ",
            code("use_effect"),
            " 鉤子中。",
        ],
        p![
            "在水合過程中，可以使用結構化元件進行服務端渲染，視圖函數將在渲染函數之前被調用多次。直到呼叫渲染函數之前，DOM 被認為是未連接的，您應該防止在呼叫 ",
            code("rendered()"),
            " 方法之前存取渲染節點。",
        ],
        h2!["範例"],
...
fn main() {
    ...
    renderer.hydrate();
        ...
        h2!["單線程模式"],
        p![
            "Yew 支援以單一執行緒進行服務端渲染，透過 ",
            code("yew::LocalServerRenderer"),
            "。這種模式適用於像 WASI 這樣的單執行緒環境。",
        ],
        code_block("rust", r#"// 使用 `wasm32-wasip1` 或 `wasm32-wasip2` 目標建置。
    ...
    println!("{}", render().await);
        ...
        admonition![
            ...
            p![
                ...
                " 功能標誌來停用 Yew 內部對特定於瀏覽器的 API 的存取。這在像 Cloudflare Worker 這樣的無伺服器平台上非常有用。",
            ],
        ],
        admonition![
            AdmonitionType::Caution,
            None,
            p![
                "服務端渲染目前是實驗性的。如果您發現了一個 bug，",
                link!["https://github.com/yewstack/yew/issues/new?assignees=&labels=bug&template=bug_report.md&title=", "請在 GitHub 回饋"],
                "。",
            ],
        ],
    ])
