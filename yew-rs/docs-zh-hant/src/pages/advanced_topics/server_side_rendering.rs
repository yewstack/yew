pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        h1![text("服務端渲染 (Server-Side Rendering)")],
        p![
        ...
        p![text("為了解決這些問題，我們可以在服務端渲染我們的網站。")],
        h2![text("工作原理")],
        p![
            text("Yew 提供了一個 "),
            code("ServerRenderer"),
...
async fn no_main() {
    ...
    println!("{}", rendered);
        ...
        h2![text("組件生命週期")],
        p![
            text("與客戶端渲染不同，元件的生命週期在服務端渲染時會有所不同。"),
        ],
        p![
            text("在元件成功第一次渲染為 "),
            code("Html"),
        ...
        h2![text("渲染 "), code("<head>"), text(" 標籤")],
        p![
            text("SSR 中的一個常見需求是渲染動態 "),
            code("<head>"),
            text(" 內容（例如 "),
            code("<title>"),
            text("、"),
            code("<meta>"),
            text("），使爬蟲和社群預覽在首次載入時能看到正確的中繼資料。"),
        ],
        p![
            code("ServerRenderer"),
            text(" 只渲染元件樹（通常對應文件的 body 部分），無法存取 "),
            code("<head>"),
            text("。因此，head 標籤必須"),
            bold![text("在伺服器端、Yew 之外")],
            text("產生，並在傳送給客戶端之前拼接到 HTML 範本中。"),
        ],
        p![
            link!["https://github.com/yewstack/yew/blob/master/examples/ssr_router/src/bin/ssr_router_server.rs", code("ssr_router"), text(" 範例")],
            text(" 展示了這個模式：伺服器從請求 URL 識別路由，產生適當的 "),
            code("<title>"),
            text(" 和 "),
            code("<meta>"),
            text(" 標籤，並將它們注入到 Trunk 產生的 "),
            code("index.html"),
            text(" 的 "),
            code("</head>"),
            text(" 之前。"),
        ...
        admonition![
            ...
            None,
            p![
                text("如需完全相容 SSR 的第三方解決方案，請使用 "),
                link!["https://docs.rs/bounce/latest/bounce/helmet/index.html", text("Bounce 的 "), code("<Helmet/>"), text(" 元件")],
                text("。"),
            ],
        ],
        ...
        admonition![
            ...
            p![
                ...
                link!["https://github.com/yewstack/yew/issues/2684", text("如果您有一個沒有"), code("<tbody>"), text(" 的"), code("<table>"), text("，瀏覽器可能會向DOM 添加一個"), code("<tbody>")],
            ],
        ],
        h2![text("水合期間的組件生命週期")],
        p![
...
fn main() {
    ...
    renderer.hydrate();
        ...
        h2![text("單線程模式")],
        p![
            text("Yew 支援以單一執行緒進行服務端渲染，透過 "),
            code("yew::LocalServerRenderer"),
            text("。這種模式適用於像 WASI 這樣的單執行緒環境。"),
        ],
    ...
    println!("{}", render().await);
        ...
        admonition![
            ...
            p![
                ...
                text(" 目標建立 SSR 應用程序，您可以使用 "),
                code("not_browser_env"),
                text(" 功能標誌來停用 Yew 內部對特定於瀏覽器的 API 的存取。這在像 Cloudflare Worker 這樣的無伺服器平台上非常有用。"),
            ],
        ],
        admonition![
            AdmonitionType::Caution,
            ...
            p![
                ...
                link!["https://github.com/yewstack/yew/issues/new?assignees=&labels=bug&template=bug_report.md&title=", text("請在 GitHub 回饋")],
                text("。"),
            ],
        ],
    ])
