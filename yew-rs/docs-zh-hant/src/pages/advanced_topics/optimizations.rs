pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        h2(vec![text("使用智慧指針")]),
        p(vec![bold(vec![
            text("注意：如果您對本節中使用的某些術語感到困惑，Rust 手冊中有一個有用的"),
            link(
        ...
        p(vec![
            ...
            code("Rc::ptr_eq"),
            text(" 的"),
            link("https://doc.rust-lang.org/stable/std/rc/struct.Rc.html#method.ptr_eq", vec![text("更多細節")]),
            text("。"),
        ]),
        p(vec![
            text("這種最佳化對於不實作 "),
        ...
        h2(vec![text("減少二進位檔案大小")]),
        ul(vec![
            li(vec![text("優化 Rust 程式碼")]),
            li(vec![
                code("cargo.toml"),
                text("（定義發布設定檔）"),
            ]),
...
lto = true"#),
        ...
        p(vec![
            ...
            link("https://doc.rust-lang.org/cargo/reference/unstable.html", vec![text("不穩定功能")]),
            text("的文檔，特別是關於"),
...
build-std-features = ["panic_immediate_abort"]"#),
        admonition(
            ...
            vec![
                p(vec![
                    ...
                    link("https://github.com/yewstack/yew/issues/2696", vec![text("這個例子")]),
        ...
        p(vec![
            ...
            link("https://rustwasm.github.io/book/game-of-life/code-size.html", vec![text("縮小 .wasm 大小")]),
        ]),
        ul(vec![
            li(vec![
                text("使用 "),
        ...
        h4(vec![text("在 yew/examples/ 中 'minimal' 範例的建置大小")]),
        p(vec![
            text("注意："),
            code("wasm-pack"),
        ...
        ul(vec![
            li(vec![
                link("https://doc.rust-lang.org/book/ch15-00-smart-pointers.html", vec![text("Rust 手冊中關於智慧型指標的章節")]),
            ]),
            li(vec![
                link("https://rustwasm.github.io/book/reference/code-size.html#optimizing-builds-for-code-size", vec![text("Rust Wasm 手冊中關於減小二進位檔案大小的資訊")]),
            ]),
            li(vec![
                link("https://doc.rust-lang.org/cargo/reference/profiles.html", vec![text("Rust 設定檔的文件")]),
            ]),
            li(vec![
                link("https://github.com/WebAssembly/binaryen", vec![text("binaryen 專案")]),
            ]),
        ]),
    ])
}
