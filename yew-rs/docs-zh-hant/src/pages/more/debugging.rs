pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        h2(vec![text("意外終止 (Panics)")]),
        p(vec![text("Yew 會自動在瀏覽器控制台中輸出意外終止日誌。")]),
        h2(vec![text("控制台日誌")]),
        p(vec![
            text("在 JavaScript 中，"),
            code("console.log()"),
...
fn main() {
    ...
    tracing::info!("Hello {}", object.as_string().unwrap());
        ...
        p(vec![
            link("https://docs.rs/tracing/latest/tracing/level_filters/index.html#compile-time-filters", vec![text("編譯時過濾器")]),
            text(" 可以用於調整詳細程度或停用日誌記錄，這應該會導致更小的Wasm 檔案。"),
        ]),
        h2(vec![text("來源映射 (Source Maps)")]),
        p(vec![
            text("有一些支援 "),
            link("https://developer.chrome.com/blog/wasm-debugging-2019/#enter-dwarf", vec![text("來源映射")]),
            text("。但是，需要一些配置。"),
        ]),
        h2(vec![text("過去的文章")]),
        p(vec![text("以下是一些關於 Rust 中 WebAssembly 偵錯狀態的過去文章。它們可能是有趣的閱讀。")]),
        p(vec![
            text("\\[Dec 2019\\] "),
            link("https://developers.google.com/web/updates/2019/12/webassembly#the_future", vec![text("Chrome DevTools 更新")]),
        ]),
        blockquote(vec![p(vec![text("這些工作還有很多要做。例如，在工具方面，Emscripten（Binaryen）和 wasm-pack（wasm-bindgen）尚未支援在它們執行的轉換上更新 DWARF 資訊。")])]),
        p(vec![
            text("\\[2020\\] "),
            link("https://rustwasm.github.io/book/reference/debugging.html#using-a-debugger", vec![text("Rust Wasm 偵錯指南")]),
        ]),
        ...
        p(vec![
            text("\\[2019\\] "),
            link("https://rustwasm.github.io/rfcs/007-2019-roadmap.html#debugging", vec![text("Rust Wasm 路線圖")]),
        ]),
        blockquote(vec![p(vec![text("偵錯很棘手，因為很多情況不在這個工作小組的掌控之中，而是取決於 WebAssembly 標準化機構和實現瀏覽器開發者工具的人。")])]),
    ])
}
