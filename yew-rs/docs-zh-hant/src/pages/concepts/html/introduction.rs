pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        ...
        admonition![
            ...
            p![
                code("html!"),
                " 巨集可能會達到編譯器的預設遞歸限制。如果遇到編譯錯誤，請在 crate 根目錄中新增類似 ",
                code("#![recursion_limit=\"1024\"]"),
                " 的屬性以解決問題。",
            ],
        ],
        h2!["標籤 (Tags) 結構"],
        p!["標籤 (Tags) 是基於 HTML 標籤。元件、元素和清單都基於此標籤語法。"],
        p![
            "標籤必須或自閉合 ",
            code("<... />"),
...
html! {
    <svg width="149" height="147" viewBox="0 0 149 147" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        h2!["Lints"],
        p![
            "如果您使用 Rust 編譯器的開發者版本編譯 Yew，巨集將警告您可能遇到的一些常見陷阱。當然，您可能需要使用穩定版編譯器（例如，您的組織可能有政策要求這樣做）進行發布構建，但即使您使用的是穩定工具鏈，運行",
            code("cargo +nightly check"),
            " 也可能會標記一些可以改進HTML 程式碼的方法。",
        ],
        p![
            "目前，這些 lint 主要與可訪問性相關。如果您有 lint 的想法，請隨時",
            link!(
...
html! { <my-element ~property="abc" /> };"#,
        ...
        h3!["特殊屬性"],
        p![
            "有一些特殊屬性不會直接影響 DOM，而是作為 Yew 虛擬 DOM 的指令。目前有兩個這樣的特殊屬性：",
            code("ref"),
            " 和 ",
            code("key"),
            "。",
        ...
        h2!["條件渲染"],
        p![
            "可以透過使用 Rust 的條件結構來條件性地渲染標記。目前只支援 ",
            code("if"),
            " 和 ",
            code("if let"),
            "。",
