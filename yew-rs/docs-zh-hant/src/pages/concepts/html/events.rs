...
html! {
    ...
    //      ^^^^^^^ event listener name
        ...
        p![
            "在啟動應用程式",
            italic!["之前"],
            "。這會加快事件處理速度，但某些元件可能會因未收到預期的事件而中斷。請謹慎使用！",
        ],
        h2!["事件委託"],
        p!["可能會讓人驚訝的是，事件監聽器並不是直接註冊在它們被渲染的元素上。相反，事件是從 Yew 應用的子樹根節點委託的。不過，事件仍然以其原生形式傳遞，並且不會創建任何合成形式。這可能會導致 HTML 監聽器中預期的事件與 Yew 中出現的事件之間的不符。"],
        ul![
            li![
                link!(
        ...
        p!["這也意味著由 Yew 註冊的事件通常會在其他事件監聽器之前觸發。"],
        h2!["具備類型的事件目標"],
        admonition![
...
fn MyComponent() -> Html {
    ...
    html! {
        ...
        h2!["可用事件的完整清單"],
        table(
            vec![vec!["偵聽器名稱".into()], vec![code("web_sys"), " 事件類型".into()]],
            vec![
