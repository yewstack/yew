pub fn page_content() -> yew_site_lib::Content {
    ...
    Content::new(vec![
        p(vec![
            ...
            text(" 為 Web API 提供綁定。這是從瀏覽器 WebIDL 產生的，這就是為什麼有些名稱如此之長，有些類型如此模糊的原因。"),
        ]),
        h2(vec![text("`web-sys` 中的特性 (features)")]),
        p(vec![
            code("web-sys"),
            text(" crate 中啟用了所有特性可能會為 Wasm 應用程式增加很多冗餘。為了解決這個問題，大多數類型都是透過啟用 features 進行控制的，這樣你只需要包含你的應用程式所需的類型。 Yew 啟用了 "),
            code("web-sys"),
            text(" 的幾個特性，並在其公共 API 中公開了一些類型。你通常需要自行將 "),
            code("web-sys"),
            text(" 新增為依賴項。"),
        ]),
        h2(vec![text("`web-sys` 中的繼承")]),
        p(vec![
            text("在"),
            link("/zh-Hant/docs/concepts/basic-web-technologies/wasm-bindgen#simulating-inheritance", vec![text("模擬繼承")]),
            text("部分，你可以了解到 Rust 通常提供了一種模擬 JavaScript 中繼承的方法。這在 "),
            code("web-sys"),
            text(" 中非常重要，因為了解一個類型上有哪些方法意味著了解它的繼承。"),
        ]),
        p(vec![
            text("這一部分將查看一個特定的元素，並使用Rust 呼叫"),
            link(
                "https://doc.rust-lang.org/std/ops/trait.Deref.html#tymethod.deref",
                vec![code("Deref::deref")],
            ),
            text(" 列出其繼承，直到該值為"),
            link("/zh-Hant/docs/concepts/basic-web-technologies/wasm-bindgen#jsvalue", vec![code("JsValue")]),
            text("。"),
        ]),
        code_block(
            "rust",
    ...
    let event_target: &EventTarget = text_area.as_ref();
        ...
        p(vec![
            ...
            link(
                ...
                vec![text("wasm-bindgen 指引中的 web-sys 繼承")],
            ),
            text("_"),
        ]),
        ...
        p(vec![
            ...
            link(
                ...
                vec![code("web_sys::Node")],
            ),
            text("。 "),
            code("NodeRef::get"),
            text(" 方法將傳回一個 "),
            code("Option<Node>"),
            text(" 值，但是，在 Yew 中，大多數情況下，您希望將此值轉換為特定元素，以便使用其特定方法。如果存在，可以使用 "),
            link("/zh-Hant/docs/concepts/basic-web-technologies/wasm-bindgen#jscast", vec![code("JsCast")]),
            text(" 對 "),
            code("Node"),
            text(" 值進行轉換，但是Yew 提供了 "),
            code("NodeRef::cast"),
            text(" 方法來執行此轉換，以方便使用，因此您不一定需要為 "),
            code("JsCast"),
            text(" 特性包含 "),
            code("wasm-bindgen"),
            text(" 依賴項。"),
...
fn with_jscast(node_ref: NodeRef) {
    if let Some(input) = node_ref
        ...
        h2(vec![text("JavaScript 重構為 Rust 的範例")]),
        p(vec![
            text("這一節展示如何將與 Web API 互動的 JavaScript 程式碼範例重寫為 Rust 中的 "),
            code("web-sys"),
            text("。"),
        ]),
        h3(vec![text("JavaScript 範例")]),
        code_block(
            "js",
...
// 我們現在需要保存 `mousemove` 閉包，以便在事件觸發時閉包仍然在記憶體中。"#,
        ...
        p(vec![
            text("這個版本更加冗長，但你可能會注意到其中的一部分是由於失敗類型提醒我們，一些函數呼叫有必須保持的不變量，否則將在 Rust 中引發 panic。另一個冗長的部分是呼叫 "),
            code("JsCast"),
            text(" 來將不同類型轉換為特定類型，以便呼叫其特定方法。"),
        ]),
        h3(vec![text("用 Yew 重寫的範例")]),
        p(vec![
            text("在Yew 中，您將主要建立 "),
            link(
            ...
            text(" 巨集中使用，因此範例將使用這種方法，而不是完全複製上面的方法："),
        ]),
        code_block_title(
            "toml",
            "Cargo.toml",
...
html! {
    <div id="mousemoveme" {onmousemove}></div>
        ...
        p(vec![
            code("web-sys"),
            text(" 是 Web API 的原始綁定，囙此在 Rust 中會有一些痛苦，因為它並不是為 Rust 或甚至强類型系統設計的，這就是社區 crate 提供了對 "),
            code("web-sys"),
            text(" 的抽象，以提供更符合 Rust 習慣的 API。"),
        ]),
        p(vec![
            text("_"),
            link("/community/external-libs", vec![text("補充依賴庫清單")]),
            text("_"),
        ]),
    ])
}
